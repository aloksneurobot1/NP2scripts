# -*- coding: utf-8 -*-
"""
Created on Sat May 31 20:28:40 2025
...
def plot_channel_psds(lfp_memmap_obj, fs, uv_scale_factor, 
                      ...
                      ripple_freq_range_for_avg, # Parameter name used in main call
                      title_prefix=""):
    nperseg = int(2 * fs) 
    fig = go.Figure()
    try:
...
def main_ripple_analysis_and_visualization():
    ...
    else: print("No sleep data loaded. Ripple-locked average plots will use 'AllData' state if any ripples found, unless STATES_TO_ANALYZE is empty.");
    ...
    if not dg_df.empty:
        ...
        plot_channel_psds(lfp_mm, fs_eff, uv_sf, dg_idxs, chan_df, 
                          ripple_f_range, # This is ripple_freq_range_for_avg in func def
        ...
        for ep_idx, (ep_start, ep_end) in enumerate(processing_epochs): 
            ...
            for (shk, area_n), ref_ch in sel_ref_chs.items():
                ...
                for i_rip, ev_dict in enumerate(ep_rips_sorted):
                    # Pop original relative samples and store them before calculating absolute
                    ...
                    if st_name_detect == "AllData" or curr_ev_st_code == st_code_detect:
                        ...
                        # Example LFP trace plotting logic (same as before)
    ...
    # --- Generate Ripple-Locked Average LFP Plots for NREM state in a User-Chosen Epoch ---
    if all_ripples_final_list and depth_column_to_use and epoch_b_samp_loaded and STATES_TO_ANALYZE:
        ...
        if nrem_state_code_for_plot is not None:
            ...
            chosen_epoch_idx_str = input(f"Enter Epoch Index for NREM ripple-locked plots (0 to {len(epoch_b_samp_loaded)-1}, or -1 to skip): ")
            if chosen_epoch_idx_str.strip() == '-1': print("Skipping NREM ripple-locked average plots by user choice.")
            else:
                try:
                    chosen_epoch_idx = int(chosen_epoch_idx_str) # Only one epoch for this specific plot type now
                    if not (0 <= chosen_epoch_idx < len(epoch_b_samp_loaded)):
                        print(f"Invalid epoch index. Please choose from {valid_epoch_indices_str}. Skipping plots.")
                    else:
                        print(f"Generating NREM ripple-locked plots for Epoch {chosen_epoch_idx}")
                        ...
                        for (shk_plot_idx, area_plot_name), ref_ch_plot_idx in sel_ref_chs.items():
                            ...
                            ripples_for_avg = ripple_df_for_plotting[
                                ...
                                (ripple_df_for_plotting['epoch_idx'] == chosen_epoch_idx) &
                            ...
                            plot_filename_base = f"{base_fname}_{nrem_state_name_for_plot}_Ep{chosen_epoch_idx}_S{shk_plot_idx}RefA{area_plot_name}C{ref_ch_plot_idx}_RippleLockedAvg"
                            
                            plot_ripple_locked_avg_lfp_plotly( 
                                plot_context_title=f"{nrem_state_name_for_plot} Ripple-Locked Avg LFP - Shank {shk_plot_idx} (Locked to {area_plot_name} RefCh {ref_ch_plot_idx}) - Epoch {chosen_epoch_idx}",
                                ...
                                lfp_filter_range=(100, 250), window_ms_around_peak=400 )
                except ValueError: print("Invalid epoch index entered. Skipping NREM ripple-locked plots.")
        else: print("    NREM state not defined in STATES_TO_ANALYZE. Cannot generate NREM ripple-locked plots.")
    ...
    elif not STATES_TO_ANALYZE : print("Skipping Ripple-Locked Average LFP plots (no sleep states defined to analyze).")
    ...
    # --- Data Saving --- 
    # ... (Data saving part remains the same) ...
    print("\n--- Data Saving ---")
