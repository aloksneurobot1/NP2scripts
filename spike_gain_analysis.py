...
It implements THREE parallel analysis methods:
    1. Deconvolution (Stark Lab, 2022) - For excitatory connections
    2. Poisson-based statistics (Stark & Abeles, 2009) - For excitatory connections
    3. Inhibition Detection (Constantinidis et al., 2002) - For inhibitory connections
...
def hollowed_median_filter(data, window_size):
    ...
    n = len(data); predictor = np.zeros(n);
...
def plot_deconvolution_results(results, epoch_idx, pre_id, post_id, output_dir):
    ...
    ax1.set_ylabel('Spike Pair Count'); ax1.set_title('Raw Cross-Correlogram (CCH)');
    ax1.grid(True, linestyle=':', alpha=0.6)
    ax_inset_pre = ax1.inset_axes([0.02, 0.65, 0.25, 0.3]);
    ax_inset_pre.bar(bin_centers_ms, results['acg_pre'], width=bin_width_ms, color='darkred'); ax_inset_pre.set_title('Pre-ACH', fontsize=9);
    ...
    ax_inset_post.bar(bin_centers_ms, results['acg_post'], width=bin_width_ms, color='darkblue'); ax_inset_post.set_title('Post-ACH', fontsize=9);
    ...
    ax2.set_ylabel('Deconvolved Counts'); ax2.set_title('Deconvolved CCH & Statistical Test');
    ax2.set_xlabel('Time Lag (ms)');
    ax2.legend(loc='upper right', fontsize=9); ax2.grid(True, linestyle=':', alpha=0.6)
    ax2.set_xlim(-25, 25)
...
# =============================================================================
# --- NEW: METHOD 3: INHIBITION (Constantinidis et al. 2002) ---
# =============================================================================
def analyze_pair_inhibition_method(
    pre_spikes, post_spikes, window_size=0.1, bin_size=0.001, # 1ms bins
    latency_window_ms=(1.0, 5.0), baseline_window_ms=(-50.0, -10.0)
):
    """
    Detects a putative inhibitory connection based on the criterion described
    in Barthó et al. (2004), citing Constantinidis et al. (2002).
    """
    cch_raw, bins = calculate_cch(pre_spikes, post_spikes, window_size, bin_size)
    bin_centers_ms = (bins[:-1] + bins[1:]) / 2 * 1000

    # 1. Calculate baseline firing rate from the specified control window
    baseline_mask = (bin_centers_ms >= baseline_window_ms[0]) & (bin_centers_ms < baseline_window_ms[1])
    if not np.any(baseline_mask):
        return {"is_inhibitory": False, "details": {"reason": "Baseline window is empty."}, "cch_raw": cch_raw, "bins": bins}

    baseline_mean = np.mean(cch_raw[baseline_mask])

    # 2. Calculate the significance threshold based on a Poisson distribution
    # This is the value at the 0.1th percentile of a Poisson distribution with a mean of baseline_mean.
    significance_threshold = poisson.ppf(0.001, baseline_mean)

    # 3. Find the indices corresponding to the synaptic latency window
    latency_indices = np.where((bin_centers_ms > latency_window_ms[0]) & (bin_centers_ms <= latency_window_ms[1]))[0]
    if len(latency_indices) < 2:
        return {"is_inhibitory": False, "details": {"reason": "Latency window too small."}, "cch_raw": cch_raw, "bins": bins}

    # 4. Check for two neighboring bins below the threshold
    is_significant = False
    trough_location_ms = np.nan
    # Iterate up to the second-to-last element in the latency window
    for i in range(len(latency_indices) - 1):
        current_idx = latency_indices[i]
        next_idx = latency_indices[i+1]

        # Check if the bins are truly neighbors in the original array
        if next_idx == current_idx + 1:
            if cch_raw[current_idx] <= significance_threshold and cch_raw[next_idx] <= significance_threshold:
                is_significant = True
                trough_location_ms = bin_centers_ms[current_idx]
                break # Exit loop once found

    details = {
        "baseline_mean": baseline_mean,
        "significance_threshold": significance_threshold,
        "trough_location_ms": trough_location_ms
    }

    return {
        "cch_raw": cch_raw, "bins": bins,
        "is_inhibitory": is_significant,
        "details": details
    }

def plot_inhibition_results(results, epoch_idx, pre_id, post_id, output_dir):
    fig, ax = plt.subplots(figsize=(10, 6))
    bin_centers_ms = (results['bins'][:-1] + results['bins'][1:]) / 2 * 1000
    bin_width_ms = (results['bins'][1] - results['bins'][0]) * 1000
    ...
    baseline_mean = results['details'].get('baseline_mean', 0)
    threshold = results['details'].get('significance_threshold', 0)

    ax.axhline(baseline_mean, color='blue', linestyle='--', label=f'Baseline Mean ({baseline_mean:.2f})')
    ax.axhline(threshold, color='red', linestyle=':', label=f'0.1th Percentile Threshold ({threshold:.2f})')

    status = "Inhibitory" if results['is_inhibitory'] else "Non-significant"

    stats_text = f"Classification: {status}"
    ax.text(0.02, 0.95, stats_text, transform=ax.transAxes, fontsize=12,
            verticalalignment='top', bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))

    ax.set_title(f'Inhibition Analysis: Epoch {epoch_idx}\n{pre_id} (Pre) -> {post_id} (Post)', fontsize=14)
    ax.set_ylabel('Spike Pair Count');
    ax.set_xlabel('Time Lag (ms)');
    ax.legend(loc='upper right', fontsize=9);
    ...
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    filename = f"Epoch{epoch_idx}_Inhibition_{pre_id}_vs_{post_id}.tiff";
    try:
        fig.savefig(output_dir / filename, dpi=150, format='tiff')
    except Exception as e:
        print(f"  Error saving plot {filename}: {e}")
...
def calculate_adjusted_r2(y_true, y_pred, n_params):
    n_samples = len(y_true); ss_res = np.sum((y_true - y_pred)**2);
...
def fit_plasticity_models(isi_data, gain_data):
    ...
    for _ in range(N_SHUFFLES_FOR_MODEL_FIT):
        ...
        if popt_s_full is not None or popt_s_dep is not None or popt_s_fac is not None:
             shuffled_r2s['full'].append(r2_s_full); shuffled_r2s['dep'].append(r2_s_dep);
             ...
             if r2_s_full > -np.inf:
                 ...
                 shuffled_r2s['diff_dep'].append(-np.inf); shuffled_r2s['diff_fac'].append(-np.inf);
...
def generate_population_plots(base_output_dir, method_name, metric_col_name):
    ...
    if method_name == 'inhibition':
        print("  - Skipping population plots: Not applicable for inhibition detection method.")
        return
    ...
    ax.set_title(f'Prevalence of Plasticity Types ({method_name.capitalize()})'); ax.set_xlabel('Best Fit Model'); ax.set_ylabel('Number of Connections')
    plt.tight_layout(); plt.savefig(base_output_dir / f"population_plasticity_prevalence_{method_name}.tiff", dpi=150); plt.close()
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5), sharey=True); fig.suptitle(f'Time Constant Distributions by Postsynaptic Cell Type ({method_name.capitalize()})')
    dep_df_all = pop_df[pop_df['best_model'] == 'depression'].dropna(subset=['tau', 'post_cell_type']); fac_df_all = pop_df[pop_df['best_model'] == 'facilitation'].dropna(subset=['tau', 'post_cell_type'])
    if not dep_df_all.empty: sns.histplot(data=dep_df_all, x='tau', hue='post_cell_type', ax=ax1, log_scale=True, element='step', fill=False, bins=20); ax1.set_title('Depression (τ_d)'); ax1.set_xlabel('τ (ms)')
    if not fac_df_all.empty: sns.histplot(data=fac_df_all, x='tau', hue='post_cell_type', ax=ax2, log_scale=True, element='step', fill=False, bins=20); ax2.set_title('Facilitation (τ_f)'); ax2.set_xlabel('τ (ms)')
    plt.tight_layout(rect=[0, 0.03, 1, 0.95]); plt.savefig(base_output_dir / f"population_tau_distributions_by_type_{method_name}.tiff", dpi=150); plt.close()
    ...
    if not dep_df.empty:
        ...
        for post_type, group_df in dep_df.groupby('post_cell_type'):
            ...
            plt.figure(figsize=(10, 8)); ax = sns.heatmap(zscore_matrix, cmap='vlag', cbar_kws={'label': metric_label})
            ...
            try:
                ...
                if pd.api.types.is_categorical_dtype(group_df['isi_bin']) and len(group_df['isi_bin'].cat.categories) == zscore_matrix.shape[1]:
                    ...
                    tick_labels = [f"{b.left:.1f}" for b in group_df['isi_bin'].cat.categories]; ax.set_xticks(np.arange(len(tick_labels)) + 0.5); ax.set_xticklabels(tick_labels, rotation=45, ha='right')
                else:
            ...
            plt.tight_layout(); plot_path = base_output_dir / f"heatmap_plot_depression_onto_{post_type}_{method_name}.tiff"; plt.savefig(plot_path, dpi=150); plt.close()
...
def run_conditional_analysis(pre_neuron, post_neuron, epoch, output_dir, method_name):
    ...
    if len(results_std) > 5:
        ...
        metric_label = 'Spike Gain' if method_name == 'deconvolution' else 'Spike Transmission Prob.'; df['best_model'] = fit_results['model'];
        df['tau'] = fit_results['tau']; df['adj_r2'] = fit_results['adj_r2']
        df['recording'] = output_dir.parts[-3]; # Go up 3 levels (comp/rec/method)
        df['comparison'] = output_dir.name; df['epoch'] = epoch['epoch_index']
...
def generate_anatomical_plot(summary_df, all_units_df, all_locations_df, output_dir, method_name, metric_col, metric_label):
    ...
    if method_name == 'inhibition':
        print("  - Skipping anatomical plot: Not applicable for inhibition detection method.")
        return
    ...
    ax.set_title(f'Master Anatomical Connectivity Plot ({method_name.capitalize()})'); ax.set_xlabel(f'{loc_x_col} (µm)'); ax.set_ylabel(f'{loc_y_col} (µm)') # Updated Labels
...
def generate_advanced_plots(master_summary_df, master_units_with_locations_df, master_locations_df_orig, base_output_dir, method_name):
    ...
    if method_name == 'inhibition':
        print("  - Skipping advanced plots: Not applicable for inhibition detection method.")
        return
    ...
    metric_label = 'Spike Gain' if method_name == 'deconvolution' else 'Spike Transmission Prob.' # Use internal consistent names after renaming in main
...
def get_method_selection_gui():
    ...
    methods = [
        ...
        ("inhibition", "Inhibition (Constantinidis et al. 2002)")
    ]
...
def analyze_pair_worker(args):
    ...
    try:
        ...
        elif method_name == 'inhibition':
            analysis_results = analyze_pair_inhibition_method(pre_epoch_spikes, post_epoch_spikes)
            status = "inhibitory" if analysis_results["is_inhibitory"] else "non_inhibitory"
            asymm_index = np.nan # Not applicable
...
def main():
    print("--- Starting Spike Gain Analysis Pipeline ---")
    ...
    for method in methods_to_run:
        if method == 'deconvolution':
            method_dir_name = "Deconvolution_Method"
        elif method == 'poisson':
            method_dir_name = "Poisson_Method"
        elif method == 'inhibition':
            method_dir_name = "Inhibition_Method"
        else:
            continue
    ...
    for rec_info in selected_recordings:
        ...
        significant_results = [res for res in all_results if res['status'] in ['significant_peak_found', 'significant_synapse', 'inhibitory']]
        ...
        for res in tqdm(significant_results, desc=f"Saving for {rec_name}"):
            ...
            try: # Add error handling for plotting/summary saving
                if method_name == 'deconvolution':
                    ...
                    if res['analysis_results']:
                        plot_inhibition_results(res['analysis_results'], res['epoch_index'], pre_id, post_id, comp_dir)
                        summary_row = {**summary_row_base,
                                       'trough_location_ms': res['analysis_results']['details']['trough_location_ms'],
                                       'baseline_mean': res['analysis_results']['details']['baseline_mean']}
                        significant_summary_data[method_name].append(summary_row)
                        rec_summary_df[method_name].append(summary_row)


                if run_conditional_analysis and method_name != 'inhibition':
    ...
    for method_name in methods_to_run:
        ...
        if significant_summary_data[method_name]:
            ...
            elif method_name == 'poisson': # poisson
