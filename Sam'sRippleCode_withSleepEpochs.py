...
def find_swr(lfp, timestamps, fs=1000, thresholds=(2, 5), durations=(20, 40, 500),
    ...
    try:
        ...
        # Envelope filter (e.g., 0.5-20Hz or 1-20Hz)
        env_low_f_filt = max(0.1, 0.5) # Ensure positive
        env_high_f_filt = min(nyquist - 0.1, 20)
        if env_low_f_filt >= env_high_f_filt: # Fallback to lowpass if bandpass range invalid
             b_env, a_env = butter(3, env_high_f_filt / nyquist, btype='low')
        else:
             b_env, a_env = butter(3, [env_low_f_filt / nyquist, env_high_f_filt / nyquist], btype='band')
    ...
    if len(envelope_raw) < 2: return [], filtered, None, None # Need at least 2 samples for zscore
    ...
    for start, end in merged:
        ...
        if max_val_z >= high_thresh:
            peak_idx_rel = np.argmin(seg_filt) 
            peak_abs = start + peak_idx_rel
    ...
    for (s, e), p, az, apow in zip(final_kept, final_peaks, final_amps_z, final_amps_pow):
        if s < len(timestamps) and p < len(timestamps) and e < len(timestamps):
            ripple_events.append({
                'start_sample': int(s), 'peak_sample': int(p), 'end_sample': int(e),
...
def calculate_advanced_ripple_features(filtered_event_lfp, fs, ripple_freq_range_tuple):
    ...
    if len(filtered_event_lfp) < max(4, 2 * (fs / ripple_freq_range_tuple[0])): 
        return peak_freq, n_cycles, peak_phase
    try: 
        nperseg_r = min(len(filtered_event_lfp), int(fs * 0.05)) 
        nperseg_r = max(4, nperseg_r) # Ensure nperseg is at least 4
        if nperseg_r > len(filtered_event_lfp): nperseg_r = len(filtered_event_lfp) # Cannot be > len
    ...
    except Exception as e: warnings.warn(f"Peak freq/cycles calc error: {e}")
    try: 
    ...
    except Exception as e: warnings.warn(f"Peak phase calc error: {e}")
    return peak_freq, n_cycles, peak_phase
...
def plot_channel_psds(lfp_memmap_obj, fs, uv_scale_factor, channel_indices_to_plot, channel_info_df, title_prefix=""):
    ...
    for i, ch_idx in enumerate(channel_indices_to_plot):
        if not (0 <= ch_idx < lfp_memmap_obj.shape[1]): 
            print(f"Warn: Ch {ch_idx} out of bounds. Skip PSD."); continue
        max_s = int(fs * 120); num_s = min(lfp_memmap_obj.shape[0], max_s)
        start_s_psd = (lfp_memmap_obj.shape[0] - num_s) // 2 if lfp_memmap_obj.shape[0] > num_s else 0 # Renamed start_s
        sig_i16 = lfp_memmap_obj[start_s_psd : start_s_psd + num_s, ch_idx]; sig_f64 = sig_i16.astype(np.float64)
        y_units = "ADC\u00b2/Hz"
        if uv_scale_factor is not None: sig_f64 *= uv_scale_factor; y_units = "\u00b5V\u00b2/Hz"
        try:
            curr_nperseg = nperseg if len(sig_f64) >= nperseg else len(sig_f64)
            if curr_nperseg == 0 : print(f"Ch {ch_idx} has 0 samples for PSD. Skip PSD."); continue # Check before welch
            freqs, psd_v = welch(sig_f64, fs=fs, nperseg=curr_nperseg)
        except ValueError as e: print(f"PSD Error Ch {ch_idx}: {e}"); continue
        psd_db = 10 * np.log10(psd_v + np.finfo(float).eps); mask = (freqs >= 1) & (freqs <= 300)
        ch_r = channel_info_df[channel_info_df['global_channel_index'] == ch_idx]
        ch_n = f"Ch {ch_idx} ({ch_r['acronym'].iloc[0]} S{ch_r['shank_index'].iloc[0]})" if not ch_r.empty else f'Ch {ch_idx}'
        fig.add_trace(go.Scatter(x=freqs[mask], y=psd_db[mask], mode='lines', line=dict(width=1.5, color=plot_colors[i % len(plot_colors)]), name=ch_n))
    fig.update_layout(title=f"{title_prefix}Welch PSD (1-300 Hz)", xaxis_title="Frequency (Hz)", yaxis_title=f"Power Spectral Density ({y_units.replace('/Hz', ' dB')})", font=dict(size=12), template="plotly_white", showlegend=True, height=600, width=1000)
    fig.show()
...
def plot_lfp_with_ripples(raw_lfp_segment_scaled, filtered_lfp_segment, envelope_segment_z,
    ...
    for ax_idx, ax_val in enumerate(axs): # Renamed ax to ax_val
...
def main_ripple_analysis_and_visualization():
    ...
    target_areas_list = ["CA1", "CA3", "CA2"]; dg_area_name_str = "DG-mo"
    ...
    if sleep_lkp:
        ...
        _get_state_func = lambda s, f_val: _get_state_internal(s,f_val,sleep_lkp['times_sec'],sleep_lkp['codes']) # Renamed f to f_val
    ...
    if not dg_df.empty:
        ...
        # Call plot_channel_psds with named arguments matching its new signature
        plot_channel_psds(lfp_memmap_obj=lfp_mm, fs=fs_eff, uv_scale_factor=uv_sf, 
                          channel_indices_to_plot=dg_idxs, channel_info_df=chan_df, 
    ...
    for shk in unique_shks:
        ...
        for area_n in target_areas_list:
            ...
            # Call plot_channel_psds with named arguments
            plot_channel_psds(lfp_memmap_obj=lfp_mm, fs=fs_eff, uv_scale_factor=uv_sf, 
                              channel_indices_to_plot=area_shk_idxs, channel_info_df=chan_df, 
    ...
    noise_ep_scaled = None # Initialize outside loop
    ...
    for st_code, st_name in states_iter.items():
        ...
        for ep_idx, (ep_start, ep_end) in enumerate(epoch_b_samp):
            ...
            if sel_noise_ch not in [None, -1] and 0 <= sel_noise_ch < n_ch:
                ...
                noise_ep_scaled = noise_ep_unsc.astype(np.float64) # Assign here
            ...
            for (shk, area_n), ref_ch in sel_ref_chs.items():
                ...
                if len(lfp_ep_unsc) == 0: print(f"      Empty LFP seg for Ch{ref_ch} ep {ep_idx}. Skip."); continue
                ...
                for i_rip, ev_dict in enumerate(ep_rips_sorted):
                    ...
                    if st_name == "AllData" or curr_ev_st_code == st_code:
                        ...
                        if len(ev_filt_lfp) > 0 : # Ensure segment is not empty for feature calculation
                             pk_f, n_cyc, pk_ph = calculate_advanced_ripple_features(ev_filt_lfp, fs_eff, ripple_f_range)
                        else:
                             pk_f, n_cyc, pk_ph = np.nan, np.nan, np.nan
    ...
    if not all_ripples_final_list: print("No ripples to save."); 
        ...
        if save_opt == 'yes':
            ...
            try:
                ...
                summary_data = []
                grouped = ripple_df_out.groupby(['state_name', 'state_code', 'epoch_idx', 'shank', 'area', 'reference_channel_idx'])
            ...
            except Exception as e: print(f"Error saving ripple data/summary: {e}")
