# -*- coding: utf-8 -*-
"""
Created on Fri May 16 11:35:20 2025
...
SWR_TARGET_REGION = 'CA2' # As per user's last run
SWR_TIME_WINDOW_MS = np.array([-100, 100])
LFP_PLOT_OVERLAY_SCALE = 0.1
PLOT_INDIVIDUAL_SWR_CSDS = True
MAX_INDIVIDUAL_SWR_PLOTS_PER_SHANK_EPOCH = 15 # Optional: Uncomment to limit plots

OUTPUT_DIR = Path("./csd_kcsd_output_swr_individual_v3") #output directory name
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

TARGET_FS_CSD = 1250.0
LFP_BAND_LOWCUT_CSD = 100.0
LFP_BAND_HIGHCUT_CSD = 250.0
NUMTAPS_CSD_FILTER = 51
...
def load_channel_info_kcsd(csv_filepath_str):
    ...
    try:
        ...
        # Base required columns for KCSD
        required_cols = ['global_channel_index', 'shank_index', 'ycoord_on_shank_um']
        
        # Check for acronym column, essential for region-based filtering
        # Assuming common names like 'acronym' or 'region'. Adjust if your CSV uses a different name.
        acronym_col_found = None
        possible_acronym_cols = ['acronym', 'region', 'brain_area', 'area'] # Add other possibilities
        for col_name in possible_acronym_cols:
            if col_name in channel_df.columns:
                acronym_col_found = col_name
                # If we rename it to 'acronym' here, downstream code can be consistent
                if col_name != 'acronym': 
                    channel_df.rename(columns={col_name: 'acronym'}, inplace=True)
                break
        
        if acronym_col_found is None and REGIONS_TO_ANALYZE: # Only error if filtering is requested and no acronym col
             print(f"Warning: No standard region/acronym column found (tried: {possible_acronym_cols}). Region filtering might not work as expected.")
             # Decide on behavior: error out, or proceed without filtering.
             # Adding a dummy if REGIONS_TO_ANALYZE is empty
             if not REGIONS_TO_ANALYZE:
                 channel_df['acronym'] = 'N/A' 
             # The check for filtering will happen in main_kcsd_analysis
        ...
        # Drop rows where essential KCSD columns are NaN
        ...
        if acronym_col_found:
            print(f"  Found region information using column: '{acronym_col_found}' (standardized to 'acronym')")
        return channel_df
    except Exception as e:
...
def main_kcsd_analysis(lfp_bin_file_path_str, lfp_meta_file_path_str, channel_info_csv_path_str,
    ...
    try:
        probe_channel_df = load_channel_info_kcsd(channel_info_csv_path_str) # Load full channel info        
        if 'acronym' not in probe_channel_df.columns:
            print("Warning: 'acronym' column not found in channel info CSV. LFP Y-axis labels might be incomplete.")            
            ...
            if SWR_ANALYSIS_ENABLED and is_target_state_epoch_for_swr and swr_timestamps_npy_path_str_in is not None:
                ...
                if len(swr_peak_timestamps_in_epoch) > 0:
                    ...
                    for i_shank, shank_id_val in enumerate(unique_shanks):
                        ...
                        # Get shank_channel_info for this specific shank
                        ...
                        if shank_channel_info_df.empty or len(shank_channel_info_df) < 2:
                        ...
                        kcsd_params_for_meta_cv = {'sigma_Sm': CSD_SIGMA_CONDUCTIVITY, 'n_src_init': num_csd_est_pts_config,
                        ...
                        try:
                            ...
                            kcsd_params_for_meta_cv['lambda_optimal'] = optimal_lambda_for_shank; kcsd_params_for_meta_cv['R_optimal_um'] = optimal_R_um_for_shank
                            ...
                            if hasattr(kcsd_cv_obj, 'cv_error_matrix') and kcsd_cv_obj.cv_error_matrix is not None:
                                ...
                                else: print("      Warning: Could not find exact match for optimal lambda/R in CV grid for plotting marker.")
                            ...
                            print(f"    Error during CV on SWR-averaged LFP (Ep{epoch_idx},Shk{shank_id_val}): {e_cv}. Cannot proceed."); traceback.print_exc(); continue
                        if optimal_lambda_for_shank is None or optimal_R_um_for_shank is None:
                             print(f"    Failed to determine optimal CV params for Shank {shank_id_val}. Skipping individual SWR CSDs."); continue
                        ...
                        for swr_idx, individual_swr_lfp_uV in enumerate(swr_lfp_segments_shank):
                            ...
                            try:
                                kcsd_single_swr_obj = ElephantKCSD1D(
                                ...
                                csd_profile_single_swr_A_m3 = kcsd_single_swr_obj.values()
                                ...
                                if PLOT_INDIVIDUAL_SWR_CSDS:
                                    # Prepare LFP for ripple-band overlay
                                    lfp_for_overlay_1_300Hz = individual_swr_lfp_uV # Shape (time_win, num_elec)
                                    lfp_ripple_band_overlay, _ = preprocess_lfp_sub_chunk(
                                        lfp_for_overlay_1_300Hz.copy(), # Process a copy
                                        fs_processed_swr,             # FS of this LFP
                                        RIPPLE_OVERLAY_LOWCUT, RIPPLE_OVERLAY_HIGHCUT,
                                        RIPPLE_OVERLAY_FILTER_NUMTAPS,
                                        1, # No further downsampling for this overlay filter
                                        fs_processed_swr
                                    )
                                    if lfp_ripple_band_overlay is None: # Fallback if filtering failed
                                        print(f"        Warning: Ripple-band filtering for overlay failed for SWR #{swr_idx}. Using 1-300Hz LFP for overlay.")
                                        lfp_ripple_band_overlay = lfp_for_overlay_1_300Hz

                                    csd_to_plot = csd_profile_single_swr_uA_mm3
                                    current_swr_peak_ts = valid_swr_peak_timestamps_for_shank[swr_idx]
                                    
                                    fig_indiv_swr, ax_csd_indiv = plt.subplots(figsize=(10, 8)) # Slightly taller for y-axis labels
                                    ...
                                    img_indiv = ax_csd_indiv.pcolormesh(plot_time_axis_ms, csd_positions_plot_um_indiv, csd_to_plot, cmap='RdBu_r', shading='gouraud', vmin=-clim_val_indiv, vmax=clim_val_indiv, rasterized=True)
                                    ...
                                    ax_csd_indiv.set_title(f'Indiv. SWR CSD & LFP ({RIPPLE_OVERLAY_LOWCUT:.0f}-{RIPPLE_OVERLAY_HIGHCUT:.0f} Hz) - Ep{epoch_idx} Shk{shank_id_val} SWR# {swr_idx}\n(Peak @{current_swr_peak_ts}) St{SWR_TARGET_STATE_CODE} Reg{SWR_TARGET_REGION}')
                                    if len(csd_positions_plot_um_indiv) > 0: ax_csd_indiv.set_ylim(max(csd_positions_plot_um_indiv), min(csd_positions_plot_um_indiv))

                                    ax_lfp_indiv = ax_csd_indiv.twinx()
                                    y_ticks_pos = []
                                    y_ticks_labels = []
                                    
                                    # electrode_coords_um_shank is sorted by depth. shank_channel_info_df is also sorted.
                                    for i_elec in range(lfp_ripple_band_overlay.shape[1]):
                                        lfp_trace = lfp_ripple_band_overlay[:, i_elec]
                                        elec_depth = electrode_coords_um_shank[i_elec] # Actual depth of the electrode
                                        
                                        # Get global channel ID and acronym from shank_channel_info_df
                                        # This assumes shank_channel_info_df rows correspond to 0..N-1 index for columns in lfp_ripple_band_overlay
                                        channel_props = shank_channel_info_df.iloc[i_elec]
                                        global_ch_id = channel_props['global_channel_index']
                                        acronym = channel_props.get('acronym', 'N/A') # Use .get for safety if 'acronym' might be missing

                                        ax_lfp_indiv.plot(plot_time_axis_ms, (lfp_trace * LFP_PLOT_OVERLAY_SCALE) + elec_depth, color='black', linewidth=0.7, alpha=0.6)
                                        y_ticks_pos.append(elec_depth)
                                        y_ticks_labels.append(f"Ch{global_ch_id} ({acronym})")

                                    ax_lfp_indiv.set_yticks(y_ticks_pos)
                                    ax_lfp_indiv.set_yticklabels(y_ticks_labels, fontsize=7) # Adjust fontsize as needed
                                    # Ensure LFP y-axis limits match CSD y-axis depth for visual alignment
                                    lfp_ax_min, lfp_ax_max = ax_csd_indiv.get_ylim() 
                                    ax_lfp_indiv.set_ylim(lfp_ax_min, lfp_ax_max) # Match and invert if CSD is inverted
                                    ax_lfp_indiv.invert_yaxis() # Match CSD depth inversion
                                    
                                    # ax_lfp_indiv.set_ylabel('LFP Traces (Ripple Band)', color='black') # Optional: if labels don't fit
                                    fig_indiv_swr.tight_layout(rect=[0, 0, 0.85, 1]) # Adjust rect for right y-axis labels + colorbar
                                    
                                    plot_prefix_indiv = OUTPUT_DIR / f"{base_filename}_individual_SWR_plots_Shk{shank_id_val}" # Subfolder per shank
                                    ...
                                    fig_indiv_swr.savefig(indiv_plot_filename, dpi=100); plt.close(fig_indiv_swr)
                                del kcsd_single_swr_obj
                            except Exception as e_indiv_kcsd: print(f"      Error processing or plotting individual SWR #{swr_idx} on Shank {shank_id_val}: {e_indiv_kcsd}"); traceback.print_exc()
