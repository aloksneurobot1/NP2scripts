# -*- coding: utf-8 -*-
"""
Created on Wed May 14 13:26:39 2025
            ...
            for i_shank, shank_id_val in enumerate(unique_shanks):
                ...
                try:
                    ...
                    shank_analog_signal = neo.AnalogSignal(
                        lfp_processed_shank_epoch_full.astype(np.float64), # Use float64 for CSD library
                        units=lfp_units_str, 
                ...
                try: 
                    ...
                    if num_csd_estimation_points > 1:
                        # gdx = (total_range) / (number_of_intervals)
                        # number_of_intervals = num_csd_estimation_points - 1
                        gdx_param = ((xmax_param - xmin_param) / (num_csd_estimation_points - 1))
                    else: 
                        gdx_param = 10.0 * pq.um 
                    if gdx_param.magnitude <= 1e-9: # Avoid zero or too small gdx if xmin=xmax
                        gdx_param = 1.0 * pq.um 
                        if num_csd_estimation_points < 1: num_csd_estimation_points = 1 # Ensure at least one source
                    ...
                    kcsd_estimator_obj = estimate_csd(
                        ...
                        method='KCSD1D', 
                        sigma=CSD_SIGMA_CONDUCTIVITY * pq.S / pq.m, # Corrected: Use sigma
                        n_src_init=num_csd_estimation_points,       # Corrected: Use n_src_init
                        xmin=xmin_param,
                        xmax=xmax_param,
                        gdx=gdx_param,
                    ...
                    # Explicitly call cross_validate on the estimator object
                    if kcsd_estimator_obj is not None and hasattr(kcsd_estimator_obj, 'cross_validate'):
                        ...
                        kcsd_estimator_obj.cross_validate(lambdas=KCSD_LAMBDAS_CV, Rs=KCSD_RS_CV_UM * pq.um)
                        print(f"      CV Optimal Lambda: {kcsd_estimator_obj.lambd}")
                        print(f"      CV Optimal R: {kcsd_estimator_obj.R * 1e6:.2f} um") # R is stored in meters in KCSD.py
                        ...
                        if hasattr(kcsd_estimator_obj, 'cv_error') and kcsd_estimator_obj.cv_error is not None:
                            ...
                            ax_cv.set_xticks(np.arange(len(KCSD_LAMBDAS_CV))) # Use KCSD_LAMBDAS_CV for ticks
                            ax_cv.set_xticklabels([f"{l:.1e}" for l in KCSD_LAMBDAS_CV], rotation=45, ha="right")
                            ax_cv.set_yticks(np.arange(len(KCSD_RS_CV_UM))) 
                            # Rs in kcsd_estimator_obj.Rs_CV are in meters (from pykcsd), KCSD_RS_CV_UM is in um
                            ax_cv.set_yticklabels([f"{r_val:.1f}" for r_val in KCSD_RS_CV_UM])

                            # Find indices of optimal lambda and R
                            # kcsd_estimator_obj.lambd and kcsd_estimator_obj.R are the selected values
                        ...
                        print("    Estimator object does not have cross_validate method or CV parameters not provided for it.")


                    csd_result_neo = kcsd_estimator_obj.estimate_csd() # Now estimate with (potentially) CV-tuned params
                    ...
                    elif hasattr(csd_result_neo, 'annotations') and 'coordinates' in csd_result_neo.annotations: 
                         csd_positions_plot_um = csd_result_neo.annotations['coordinates'].rescale('um').magnitude.flatten()
                    else: 
                         csd_positions_plot_um = np.linspace(electrode_coords_um_shank.min(), electrode_coords_um_shank.max(), csd_data_matrix.shape[0])


                    if csd_data_matrix.size == 0: 
                        # ... (plot error and continue)
                        pass # Already handled by the plotting logic below
                    ...
                    if csd_data_matrix.size == 0 or np.all(np.isnan(abs_csd)) or not np.any(np.isfinite(abs_csd)):
