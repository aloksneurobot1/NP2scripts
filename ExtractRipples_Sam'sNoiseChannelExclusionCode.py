...
def get_voltage_scaling_factor(meta):
    ...
    try:
        ...
        if probe_type in [21, 24, 2013]: # NP2.0 1/4-shank, NP2.0 SS (Type 24)
            lfp_gain = 80.0
...
# --- NEW Helper Function for CA1 PSD-based Reference Channel Selection ---
def select_ca1_ref_by_psd_per_shank(
    lfp_filepath,
    meta_filepath,
    n_total_channels,
    n_total_samples,
    shank_ca1_channel_indices,
    ...
    """
    Selects the best CA1 reference channel for a given shank based on PSD analysis.
    LFP is: 1. Voltage scaled. 2. Downsampled to target_psd_fs (e.g., 1250 Hz).
    3. Bandpass filtered (Sam's code Step 1: 100-250 Hz). 4. PSD calculated via Welch.
    """
    ...
    if meta is None:
        warnings.warn(f"PSD Sel: Could not read meta file {meta_filepath}. Cannot select CA1 ref channel.")
        return None

    fs_orig_str = meta.get('imSampRate') or meta.get('niSampRate')
    if fs_orig_str is None:
        warnings.warn(f"PSD Sel: Sampling rate not found in meta file {meta_filepath}. Cannot select CA1 ref channel.")
        return None
    ...
    if not shank_ca1_channel_indices:
        warnings.warn("PSD Sel: No CA1 channel indices provided for PSD selection on this shank.")
        return None

    if fs_orig <= 0:
        warnings.warn(f"PSD Sel: Invalid original sampling rate ({fs_orig} Hz). Cannot select CA1 ref channel.")
        return None

    q = 1
    channel_specific_current_fs = fs_orig # Default if no downsampling
    if fs_orig > target_psd_fs:
        q_float = fs_orig / target_psd_fs
        q = int(round(q_float))
        if q <= 0: q = 1 # Should not happen if target_psd_fs is reasonable
    
    if q > 1:
        channel_specific_current_fs = fs_orig / q
    else: # q == 1
        channel_specific_current_fs = fs_orig


    duration_to_load_sec = 60 # Load 1 minute of data for PSD
    samples_to_load_orig_fs = int(duration_to_load_sec * fs_orig)
    if n_total_samples <= 0:
        warnings.warn(f"PSD Sel: Total samples in recording is {n_total_samples}. Cannot load data.")
        return None
    ...
    if samples_to_load_orig_fs < fs_orig * 5 : # Need at least a few seconds
        warnings.warn(f"PSD Sel: Not enough data to load sufficient segment. Required: {fs_orig*5}, available: {samples_to_load_orig_fs}")
        return None
        
    lfp_memmap = None
    try:
        lfp_memmap = np.memmap(lfp_filepath, dtype='int16', mode='r', shape=(n_total_samples, n_total_channels))
        
        for ch_idx in shank_ca1_channel_indices:
            if not (0 <= ch_idx < n_total_channels):
                warnings.warn(f"PSD Sel: Ch {ch_idx} out of bounds. Skipping.")
                continue

            lfp_segment_raw = lfp_memmap[start_sample_offset : start_sample_offset + samples_to_load_orig_fs, ch_idx].astype(np.float64)

            if uv_scale_factor is not None:
                lfp_segment_scaled = lfp_segment_raw * uv_scale_factor
            else:
                lfp_segment_scaled = lfp_segment_raw
            del lfp_segment_raw

            lfp_processed_for_psd = lfp_segment_scaled
            current_fs_for_channel = fs_orig # Initialize effective fs for this channel
            ...
            if q > 1: # Downsample if q > 1
                if len(lfp_segment_scaled) > q * 20: # Check if enough samples for decimation
                    ...
                    current_fs_for_channel = fs_orig / q
                else:
                    warnings.warn(f"PSD Sel: Ch {ch_idx}: Not enough data for decimation (q={q}). Using data at {fs_orig} Hz.")
                    lfp_processed_for_psd = lfp_segment_scaled # Use original scaled if decimation fails
                    current_fs_for_channel = fs_orig # Keep original fs
            else: # No downsampling (q=1)
                lfp_processed_for_psd = lfp_segment_scaled
                current_fs_for_channel = fs_orig
            del lfp_segment_scaled


            # Apply Sam's Bandpass Filter (100-250 Hz Butterworth, order 3)
            freq_range_sam = (100.0, 250.0) 
            nyquist_filt = current_fs_for_channel / 2.0
            
            actual_highcut = min(freq_range_sam[1], nyquist_filt * 0.99) # Ensure highcut < Nyquist
            actual_lowcut = freq_range_sam[0]
            if actual_lowcut >= actual_highcut: # Ensure lowcut < highcut
                actual_lowcut = actual_highcut * 0.5 # Fallback if lowcut is too high
                warnings.warn(f"PSD Sel: Ch {ch_idx}: Adjusted lowcut for Sam's filter to {actual_lowcut:.2f} due to highcut {actual_highcut:.2f} at fs {current_fs_for_channel:.2f}Hz.")

            if actual_lowcut < actual_highcut and nyquist_filt > actual_highcut :
                try:
                    b, a = butter(3, [actual_lowcut, actual_highcut], btype='band', fs=current_fs_for_channel)
                    lfp_processed_for_psd = scipy_filtfilt(b, a, lfp_processed_for_psd)
                except ValueError as ve_filt:
                    warnings.warn(f"PSD Sel: Ch {ch_idx}: Error applying Sam's bandpass filter (butter): {ve_filt}. Using un-bandpassed LFP for PSD.")
            else:
                warnings.warn(f"PSD Sel: Ch {ch_idx}: Cannot apply Sam's bandpass filter due to invalid freq range [{actual_lowcut:.2f}, {actual_highcut:.2f}] vs Nyquist {nyquist_filt:.2f}. Using un-bandpassed LFP for PSD.")
            
            nperseg_welch = int(2 * current_fs_for_channel) # 2 seconds window for Welch
            if len(lfp_processed_for_psd) < nperseg_welch:
                 warnings.warn(f"PSD Sel: Ch {ch_idx}: Not enough data after processing for Welch PSD ({len(lfp_processed_for_psd)} samples for fs={current_fs_for_channel:.2f}Hz). Skipping.")
                 continue
            
            try:
                frequencies, psd_values = welch(lfp_processed_for_psd, fs=current_fs_for_channel, nperseg=nperseg_welch)
            except ValueError as ve_welch: # Catch Welch specific errors
                warnings.warn(f"PSD Sel: Ch {ch_idx}: Error during Welch calculation: {ve_welch}. Skipping.")
                continue
            del lfp_processed_for_psd
            
            psd_db = 10 * np.log10(psd_values + np.finfo(float).eps) # Add epsilon to avoid log(0)
            
            # Z-score PSD dB within the 100-250 Hz band for metric
            metric_freq_mask = (frequencies >= freq_range_sam[0]) & (frequencies <= freq_range_sam[1])
            ...
            if not np.isnan(metric) and metric > max_psd_metric:
                max_psd_metric = metric
                selected_channel_idx = ch_idx
            gc.collect()

    except FileNotFoundError:
        warnings.warn(f"PSD Sel: LFP file not found: {lfp_filepath}")
        return None
    except Exception as e:
        warnings.warn(f"PSD Sel: Error during CA1 ref channel selection: {e}")
        traceback.print_exc()
    finally:
        if lfp_memmap is not None and hasattr(lfp_memmap, '_mmap') and lfp_memmap._mmap is not None:
            try:
                lfp_memmap._mmap.close()
            except Exception:
                pass 
        gc.collect()
        
    if selected_channel_idx is not None:
        print(f"  PSD Sel: Selected CA1 ref channel for shank via PSD: {selected_channel_idx} (Metric: {max_psd_metric:.2f})")
    else:
        print(f"  PSD Sel: Could not select CA1 ref channel via PSD for this shank (Max metric found: {max_psd_metric}).")
...
def run_ripple_analysis(lfp_filepath, meta_filepath, channel_info_filepath,
    ...
    try:
        ...
        # ... (Ripple rate calculation logic remains the same as in original)
        ripple_rates_by_state_epoch = {state: {} for state in STATES_TO_ANALYZE}
        state_duration_in_epoch = {state: {} for state in STATES_TO_ANALYZE} 
        ripple_counts_by_state_epoch = {state: {} for state in STATES_TO_ANALYZE} 
        ...
        # ... (SPW-R Coincidence logic remains the same, uses ripple_events_by_state_global and spw_events_by_state_global)
        ...
        for state_code, state_name in STATES_TO_ANALYZE.items():
            ...
            for ch in ca1_indices:
                # Use the already state-filtered and noise-filtered (for ripples) global lists
                ...
                coincident_ripples = []
                # Efficient check: sort both by start time if not already
                # ripples_filt.sort(key=lambda x: x['start_sample']) # Already sorted by peak_sample
                # spws_filt.sort(key=lambda x: x['start_sample']) # Not sorted, but simple iteration is fine for moderate N
        ...
        # For CA2, CA3, use original logic with ALL detected ripples (before state/noise filtering)
        # This ensures reference site selection is robust and not biased by state-specific filtering if desired
        ...
        for shank in shanks:
            shank_ch_df = region_channels_df[region_channels_df['shank_index'] == shank]
            ref_sites_shank = {}
            print(f"  Processing Shank {shank} for reference sites...")
            for region in target_regions: # ['CA1', 'CA3', 'CA2']
                ...
                ref_ch_for_region_shank = -1
                if region == 'CA1':
                    print(f"    Determining CA1 reference for Shank {shank} using PSD method...")
                    shank_ca1_indices = reg_shank_ch_df['global_channel_index'].astype(int).tolist()
                    if shank_ca1_indices:
                        ref_ch_for_region_shank = select_ca1_ref_by_psd_per_shank(
                            lfp_filepath=lfp_filepath,
                            meta_filepath=meta_filepath,
                            n_total_channels=n_channels,
                            n_total_samples=n_samples_orig,
                            shank_ca1_channel_indices=shank_ca1_indices,
                            readMeta_func=readMeta, # Pass the imported readMeta
                            get_voltage_scaling_factor_func=get_voltage_scaling_factor, # Pass helper
                            target_psd_fs=1250.0 
                        )
                    else: print(f"    No CA1 channels found on Shank {shank} for PSD analysis.")
                else: # For CA2, CA3 - Retain existing logic (based on max mean power of ALL ripples)
                    print(f"    Determining {region} reference for Shank {shank} using event power method...")
                    max_pow, current_ref_ch = -1, -1
                    for _, row in reg_shank_ch_df.iterrows():
                        ch = int(row['global_channel_index'])
                        evs_ref = _all_ripple_global_temp_for_ref.get(ch, []) # Use all ripples for CA2/CA3 ref
                    ...
                    if ref_ch_for_region_shank != -1:
                        print(f"      Selected {region} ref ch for Shank {shank}: {ref_ch_for_region_shank} (Avg Ripple Power: {max_pow:.2f})")
                
                if ref_ch_for_region_shank is not None and ref_ch_for_region_shank != -1:
                    ref_sites_shank[region] = ref_ch_for_region_shank
                else: print(f"    Could not determine reference for {region} on Shank {shank}.")
            if ref_sites_shank: reference_sites[shank] = ref_sites_shank
        
        del _all_ripple_global_temp_for_ref # Clean up temp dict
        reference_channel_set = set(ch_idx for shank_data in reference_sites.values() for ch_idx in shank_data.values())
        ...
        # This step remains to create `averaged_timestamps_by_state_epoch` for co-occurrence and spectrograms
        ...
        for state_code, state_name in STATES_TO_ANALYZE.items():
            print(f"  Processing state: {state_name}")
            # Determine epochs that have any data for this state (from ripple_events_by_state_global or ca1_spwr_events_by_state_global)
            ...
            for epoch_idx in sorted(list(relevant_epochs_for_state)):
                ...
                for region in target_regions:
                    # Get reference channels for this specific region from the new `reference_sites`
                    regional_ref_indices_for_current_region = [
                        sites[region] for shank, sites in reference_sites.items() if region in sites
                    ]
                    if not regional_ref_indices_for_current_region: continue # Skip if no ref channel for this region

                    epoch_state_region_times = []
                    event_type_desc = "Unknown" # For logging
                    if region == 'CA1':
                        event_type_desc = "SPW Trough (SPW-R)"
                        for ch_ref in regional_ref_indices_for_current_region: # Iterate over ref channels for CA1
                            # Use ca1_spwr_events_by_state_global which contains SPW-R events
                            ch_spwr_events = ca1_spwr_events_by_state_global[state_code].get(ch_ref, [])
                            # Filter these events for the current epoch_idx
                        ...
                        for ch_ref in regional_ref_indices_for_current_region: # Iterate over ref channels for CA2/CA3
                            # Use ripple_events_by_state_global
                    ...
                    if epoch_state_region_times:
                        # Pool timestamps from all reference channels for this region, sort, and unique
        ...
        # This logic should be fine as it uses averaged_timestamps_by_state_epoch
        cooccurrence_results_by_state_epoch = {state: {} for state in STATES_TO_ANALYZE}
            ...
            for epoch_idx in averaged_timestamps_by_state_epoch.get(state_code, {}):
                ...
                for ref_region, target_check_regions in cooccurrence_pairs.items(): 
                    ...
                    for tr_reg in target_check_regions: # e.g. 'CA1' or 'CA3'
                        # Find the single reference channel for this target_region 'tr_reg'
                        # Assumes one ref channel per region per shank, and we take the first shank if multiple.
                        # Or, if you want to check against all ref channels of tr_reg, this logic needs adjustment.
                        # Current logic takes the first available reference channel for that target region.
                        target_ch_for_tr_reg_list = [sites[tr_reg] for shank, sites in reference_sites.items() if tr_reg in sites]
                    ...
                    for target_reg_str, target_ch_idx in target_site_indices.items():
                        ...
                        # Get events from the specific target_ch_idx for the current epoch and state
                        ...
                        for ref_t in ref_timestamps_epoch_state: # ref_t is SPW trough for CA1 ref, ripple peak for CA2/3 ref
                            ...
                            # The events found are target region ripple peaks (or CA1 SPW-R peaks)
        ...
        for state_code, state_name in STATES_TO_ANALYZE.items():
            ...
            for epoch_idx in averaged_timestamps_by_state_epoch.get(state_code, {}):
                ...
                for region, timestamps_epoch_state_list in averaged_timestamps_by_state_epoch[state_code][epoch_idx].items():
                    regional_ref_indices_for_spec = [sites[region] for shank, sites in reference_sites.items() if region in sites] 
        ...
        # ... (This filtering for CSV remains the same) ...
        ripple_events_by_state_global_ref = {state: {} for state in STATES_TO_ANALYZE}
        # spw_events_by_state_global_ref = {state: {} for state in STATES_TO_ANALYZE} # Not used for CSV directly
        ca1_spwr_events_by_state_global_ref = {state: {} for state in STATES_TO_ANALYZE}
        # ripple_events_by_state_epoch_ref = {state: {} for state in STATES_TO_ANALYZE} # Not used for CSV
        # spw_events_by_state_epoch_ref = {state: {} for state in STATES_TO_ANALYZE} # Not used for CSV
        ...
        if PLOT_LFP_EXAMPLES:
            ...
            # ... (Plotting logic remains the same, uses *_ref dictionaries) ...
            ...
            for state_code, state_name in STATES_TO_ANALYZE.items():
                ...
                # Prioritize SPW-R for CA1 channels if they exist
                for ch_idx_ref in reference_channel_set:
                    ...
                    elif ch_idx_ref in ripple_events_by_state_global_ref[state_code]: # Fallback to ripples for CA1 or other regions
                ...
                for ch_idx_plot, (events_list_plot, event_label_prefix_plot) in events_to_plot_source.items():
                    ...
                    # Sort events by peak power Z-score to plot most prominent ones, if available
                    try: events_list_plot.sort(key=lambda x: x.get('peak_power_zscore', -np.inf), reverse=True)
                    except: pass # If z-score not present or error sorting
        ...
        # ... (CSV saving logic remains the same, uses *_ref dictionaries) ...
        ...
        for state_code, state_name in STATES_TO_ANALYZE.items():
            ...
            for ch_idx_csv, events_csv in ripple_events_by_state_global_ref[state_code].items(): # Iterate through ref channels only
        ...
        # ... (Ripple rate CSV saving logic remains the same) ...
        rate_data_for_csv = []
        ...
        # Save the NEW consolidated timestamps file
        ...
        for state_code, state_name in STATES_TO_ANALYZE.items():
            ...
            state_suffix_analysis = f"_{state_name}" # For analysis outputs not strictly event lists

            # Save event lists (filtered to reference channels)
            ...
            # np.save(output_path / f'{output_filename_base}_spw_events{state_suffix_refch}_global.npy', spw_events_by_state_global_ref[state_code], allow_pickle=True) # SPW not filtered by ref usually
            ...
            # np.save(output_path / f'{output_filename_base}_ripple_events{state_suffix_refch}_by_epoch.npy', ripple_events_by_state_epoch_ref[state_code], allow_pickle=True)
            # np.save(output_path / f'{output_filename_base}_spw_events{state_suffix_refch}_by_epoch.npy', spw_events_by_state_epoch_ref[state_code], allow_pickle=True)
            
            # REMOVED saving of old averaged_timestamps_by_state_epoch[state_code]
            # np.save(output_path / f'{output_filename_base}_ripple_timestamps{state_suffix_analysis}_by_epoch.npy', averaged_timestamps_by_state_epoch[state_code], allow_pickle=True)
