# -*- coding: utf-8 -*-
"""
Created on Thu May  1 18:34:53 2025
...
def run_ripple_analysis(lfp_filepath, meta_filepath, channel_info_filepath,
    ...
    try:
        ...
        if sleep_state_data is not None:
             ...
             warnings.warn("No sleep state data loaded - cannot separate analyses by state.")
             # Exit or proceed without state separation? Exiting as per request's focus.
        ...
        # 6. Filter Events by State and Aggregate
        print("\n--- 6. Filtering Events by Sleep State & Aggregating ---")
        print(f"Filtering detected events to keep states: {STATES_TO_ANALYZE}...")
        for state_code, state_name in STATES_TO_ANALYZE.items():
            ...
            for epoch_idx in all_ripple_events_by_epoch:
                ripple_events_by_state_epoch[state_code][epoch_idx] = {}
                for ch_idx, events in all_ripple_events_by_epoch[epoch_idx].items():
                    filtered_events = [ev for ev in events if state_lookup(ev['peak_sample'], fs_orig) == state_code]
                    if filtered_events:
                        ripple_events_by_state_epoch[state_code][epoch_idx][ch_idx] = filtered_events
                        # Add epoch index before global aggregation
                        for ev in filtered_events: ev['epoch_idx'] = epoch_idx
                        ripple_events_by_state_global[state_code].setdefault(ch_idx, []).extend(filtered_events)
            ...
            print(f"Filtered and aggregated events for state: {state_name} ({state_code})")


        # 7. SPW-R Coincidence Check (Separately for NREM and Awake)
        ...
        # ... (Logic unchanged - uses temporary aggregation of ALL events) ...
        ...
        for state_code, state_name in STATES_TO_ANALYZE.items():
            ...
            for epoch_idx in ripple_events_by_state_epoch.get(state_code, {}): # Use state-specific dict
        ...
        # 10. Co-occurrence Detection per State per Epoch
        ...
        ref_region = 'CA2'; target_check_regions = ['CA1', 'CA3']
        for state_code, state_name in STATES_TO_ANALYZE.items():
             print(f"  Processing state: {state_name}")
             ...
             for epoch_idx in averaged_timestamps_by_state_epoch.get(state_code, {}):
                 ...
                 # print(f"      Using {len(ref_timestamps_epoch_state)} {ref_region} timestamps as reference.")
                 ...
                 for target_reg, target_ch in target_site_indices.items():
                     ...
                     elif target_reg == 'CA3': ch_ripple_events = ripple_events_by_state_global[state_code].get(target_ch, []); target_event_times_epoch_state = np.array([ev['peak_sample'] for ev in ch_ripple_events if ev.get('epoch_idx') == epoch_idx])
        ...
        print(f"Results saved to {output_path} with prefix '{output_filename_base}' (State-Specific)")
