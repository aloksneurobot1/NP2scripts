...
MIN_SPIKES_FOR_CONDITIONAL_ANALYSIS = 100 
RUN_CONDITIONAL_ANALYSIS = True 
N_SHUFFLES_FOR_MODEL_FIT = 1000 # Warning: High values are computationally expensive
...
# --- NameError FIX: Restoring the missing conditional analysis helper functions ---
...
def main():
    ...
    for unit_path in data_dir.glob('good_clusters_processed_*_CellExplorerACG.npy'):
        try:
            ...
            if ts_path.exists(): recording_pairs.append({"name": name_part, "units": unit_path, "timestamps": ts_path}); print(f"  - Found match: {name_part}")
