...
def load_lfp_data_memmap(file_path, meta_path, data_type='int16'):
    ...
    try:
        ...
        if srate_key not in meta: print(f"Error: SampRate key missing."); return None, None, 0, 0, None
...
def plot_channel_psds(lfp_memmap_obj, fs, uv_scale_factor, 
                      ...
                      ripple_freq_range_tuple, # ADDED this argument
                      title_prefix=""):
    nperseg = int(2 * fs) 
    fig = go.Figure()
    try:
...
def main_ripple_analysis_and_visualization():
    ...
    ripple_thr = (2, 4); ripple_dur = (20, 40, 500); ripple_f_range = (100, 250) # This is a tuple
    ...
    if not dg_df.empty:
        ...
        plot_channel_psds(lfp_mm, fs_eff, uv_sf, dg_idxs, chan_df, ripple_f_range, title_prefix=f"{dg_area_name_str} Channels ") # Pass ripple_f_range
    ...
    for shk in unique_shks:
        ...
        for area_n in target_areas_list:
            ...
            plot_channel_psds(lfp_mm, fs_eff, uv_sf, area_shk_idxs, chan_df, ripple_f_range, title_prefix=f"S{shk} {area_n} ") # Pass ripple_f_range
            while True:
                try:
            ...
            try:
                ...
                for col in group_by_cols:
                    if col not in ripple_df_out.columns:
                        print(f"Warning: Groupby column '{col}' not found in detailed ripple data. Summary might be incomplete.")
                        # Potentially add the column with a default if appropriate, or skip grouping by it
                
                # Convert types for groupby if necessary, e.g., ensure numeric are numeric
                # ripple_df_out['shank'] = pd.to_numeric(ripple_df_out['shank'], errors='coerce')
                # ripple_df_out['reference_channel_idx'] = pd.to_numeric(ripple_df_out['reference_channel_idx'], errors='coerce')
