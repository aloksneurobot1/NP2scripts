# -*- coding: utf-8 -*-
"""
Created on Fri Mar 14 10:42:56 2025
...
def compute_emg(data, fs, window_size, step_size, n_ica_components=10, ica_epochs_duration=240, num_channels_for_ica=48): # Added num_channels_for_ica parameter
    """
    ...
    return emg_segmented, ica, ica_source_signals 
...
def score_sleep_states(pc1, theta_dominance, emg, buffer_size=10, fixed_emg_threshold=None, speed_csv_path=None, video_fps=30, video_start_offset_seconds=0, speed_threshold=1.5, use_emg=True):
    ...
    print("---Threshold Values---") # Debug print 
...
# Get directory paths from user using a browser
root = Tk()
root.withdraw()
...
if lfp_bin_meta_dir is None or output_dir is None:
    print("LFP binary/meta directory or output directory not selected. Exiting.")
    sys.exit()

# Find LFP binary files and their corresponding meta files
lfp_files = sorted(list(lfp_bin_meta_dir.glob('*.bin')))

if not lfp_files:
    print(f"No LFP binary files found in: {lfp_bin_meta_dir}")
    sys.exit()

print(f"Found the following LFP binary files: {[f.name for f in lfp_files]}")

# Iterate over each LFP recording
for lfp_file_path in lfp_files:
    print(f"\n--- Processing recording: {lfp_file_path.name} ---")
    meta_file_path = lfp_file_path.with_suffix('.meta')

    if not meta_file_path.exists():
        print(f"Warning: Corresponding meta file not found for {lfp_file_path.name}. Skipping this recording.")
        continue

    # Try to find corresponding timestamp file (assuming same base name)
    timestamp_file_path = None
    if timestamps_dir:
        base_name = lfp_file_path.stem
        possible_timestamp_files = list(timestamps_dir.glob(f'{base_name}.*')) # Adjust pattern as needed
        if possible_timestamp_files:
            timestamp_file_path = possible_timestamp_files[0] # Take the first one if multiple found
            print(f"Found corresponding timestamp file: {timestamp_file_path.name}")
        else:
            print(f"No timestamp file found for {lfp_file_path.name} in {timestamps_dir}.")

    # Try to find corresponding speed file (assuming same base name)
    current_speed_file_path = None
    if speed_files_dir:
        base_name = lfp_file_path.stem
        possible_speed_files = list(speed_files_dir.glob(f'{base_name}.csv')) # Assuming .csv extension
        if possible_speed_files:
            ...
            print(f"No speed file found for {lfp_file_path.name} in {speed_files_dir}.")

    # Load data
    try:
        data = read_bin_data(lfp_file_path, meta_file_path)
    except Exception as e:
        print(f"Error reading data for {lfp_file_path.name}: {e}. Skipping this recording.")
        continue

    # Select last num_channels channels
    if data.shape[1] < num_channels:
        print(f"Warning: Less than {num_channels} channels in {lfp_file_path.name}. Skipping this recording.")
        continue
    ...
    # Save sleep states to the specified output directory
    output_filename = f'{lfp_file_path.stem}_sleep_states.npy'
    ...
    if calculate_emg and ica_components is not None: # Only plot if ICA was actually run and components exist
        ...
        for i in range(num_ics_to_plot):
            ...
            plt.title(f"IC {i} - {lfp_file_path.stem}")
        ...
        for i in range(num_ics_to_plot):
            ...
            plt.title(f"IC {i} - Power Spectrum - {lfp_file_path.stem}")
...
print("Script execution completed. Please check script_output.txt and the generated sleep state files.")