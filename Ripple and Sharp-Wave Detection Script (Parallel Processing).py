# -*- coding: utf-8 -*-
"""
Created on Wed Apr 30 17:49:32 2025
...
# Use most cores, leaving one or two free for system responsiveness
...
def _calculate_baseline_worker(ch_idx, lfp_filepath, meta_filepath, non_rem_periods_samples, fs, n_channels):
    """Worker function to calculate baseline stats for a single channel."""
    # print(f"  Baseline Worker: Processing Channel {ch_idx}...") # Can be too verbose
    lfp_data_memmap_worker = None
    try:
        # Each worker creates its own memmap view
        shape = (None, n_channels) # Shape needs n_samples, get from meta? Or pass n_samples?
        # Re-read n_samples here or pass it
        ...
        lfp_ch_full = lfp_data_memmap_worker[:, ch_idx].astype(np.float64)

        lfp_ch_ripple_filtered_full = apply_fir_filter(lfp_ch_full, RIPPLE_FILTER_LOWCUT, RIPPLE_FILTER_HIGHCUT, fs)
        del lfp_ch_full # Free memory early
        if lfp_ch_ripple_filtered_full is None: return ch_idx, None

        ripple_power_full = calculate_instantaneous_power(lfp_ch_ripple_filtered_full)
        del lfp_ch_ripple_filtered_full
        if ripple_power_full is None: return ch_idx, None

        baseline_mean, baseline_sd = get_baseline_stats(ripple_power_full, fs, non_rem_periods_samples)
        del ripple_power_full
        # print(f"  Baseline Worker: Channel {ch_idx} complete.")
        ...
        return ch_idx, None # Return None on error
    finally:
        # Ensure memmap is closed in the worker
        if lfp_data_memmap_worker is not None and hasattr(lfp_data_memmap_worker, '_mmap'):
            try: lfp_data_memmap_worker._mmap.close()
            except Exception: pass
        gc.collect() # Encourage garbage collection in worker


def _detect_events_worker(ch_idx, ch_region, epoch_start_sample, epoch_end_sample,
                         lfp_filepath, n_samples, n_channels, baseline_stats, fs):
    """Worker function to detect events for a single channel within an epoch."""
    # print(f"  Event Worker: Proc Ch {ch_idx} Epoch {epoch_start_sample}-{epoch_end_sample}...") # Verbose
    lfp_data_memmap_worker = None
    ripple_events = []
    spw_events = []
    try:
        ...
        lfp_ch_epoch = lfp_data_memmap_worker[epoch_start_sample:epoch_end_sample, ch_idx].astype(np.float64)

        # --- Ripple Detection ---
        lfp_ch_ripple_filtered = apply_fir_filter(lfp_ch_epoch, RIPPLE_FILTER_LOWCUT, RIPPLE_FILTER_HIGHCUT, fs)
        if lfp_ch_ripple_filtered is not None:
            ripple_power = calculate_instantaneous_power(lfp_ch_ripple_filtered)
            if ripple_power is not None:
                baseline_mean, baseline_sd = baseline_stats
                if baseline_sd > 0: # Check for valid baseline SD
                    ...
                    ripple_starts_rel, ripple_ends_rel = detect_events(
                        ripple_power, fs, threshold_high=detection_threshold, threshold_low=expansion_threshold,
                    ...
                    for start_rel, end_rel in zip(ripple_starts_rel, ripple_ends_rel):
                        if start_rel < 0 or end_rel >= len(lfp_ch_ripple_filtered): continue
                        ...
                        if peak_power_idx_rel != -1 and trough_idx_rel != -1:
                            ...
                            ripple_events.append({
                                ...
                                'peak_power': peak_power_val, 'duration_ms': (end_rel - start_rel + 1) / fs * 1000
                            })
                # Clean up ripple intermediates
                del ripple_power
            del lfp_ch_ripple_filtered

        # --- SPW Detection (CA1 only) ---
        if ch_region == 'CA1':
            lfp_ch_spw_filtered = apply_fir_filter(lfp_ch_epoch, SPW_FILTER_LOWCUT, SPW_FILTER_HIGHCUT, fs)
            if lfp_ch_spw_filtered is not None:
                spw_signal_mean = np.nanmean(lfp_ch_spw_filtered)
                spw_signal_sd = np.nanstd(lfp_ch_spw_filtered)

                if spw_signal_sd > 0 and np.isfinite(spw_signal_sd):
                    spw_detection_threshold = SPW_DETECTION_SD_THRESHOLD * spw_signal_sd
                    spw_expansion_threshold = 1.0 * spw_signal_sd

                    spw_starts_rel, spw_ends_rel = detect_events(
                         np.abs(lfp_ch_spw_filtered - spw_signal_mean), fs, threshold_high=spw_detection_threshold,
                    ...
                    for start_rel, end_rel in zip(spw_starts_rel, spw_ends_rel):
                         if start_rel < 0 or end_rel >= len(lfp_ch_spw_filtered): continue
                ...
                del lfp_ch_spw_filtered

        del lfp_ch_epoch # Free epoch segment memory
        # print(f"  Event Worker: Ch {ch_idx} Epoch {epoch_start_sample}-{epoch_end_sample} complete.")
        return ch_idx, ripple_events, spw_events

    except Exception as e:
        print(f"Error in event worker for channel {ch_idx}, epoch {epoch_start_sample}-{epoch_end_sample}: {e}")
        # traceback.print_exc()
        return ch_idx, [], [] # Return empty lists on error
    finally:
        if lfp_data_memmap_worker is not None and hasattr(lfp_data_memmap_worker, '_mmap'):
            try: lfp_data_memmap_worker._mmap.close()
            except Exception: pass
...
def _calculate_spectrogram_worker(ts_sample, spec_ch_idx, lfp_filepath, n_samples, n_channels, fs):
    ...
    try:
        ...
        shape = (n_samples, n_channels)
        lfp_data_memmap_worker = np.memmap(lfp_filepath, dtype='int16', mode='r', shape=shape)
        lfp_spec_ch_full = lfp_data_memmap_worker[:, spec_ch_idx].astype(np.float64)

        # Calculate spectrogram
        spec_window_samples = int(SPECTROGRAM_WINDOW_MS * fs / 1000)
        spec = calculate_cwt_spectrogram(
            lfp_spec_ch_full, fs, SPECTROGRAM_FREQS,
            timestamp_samples=ts_sample,
            window_samples=spec_window_samples
        )
        del lfp_spec_ch_full
        return spec # Return the calculated spectrogram (or NaN array on error)

    except Exception as e:
        # print(f"Error in spectrogram worker for timestamp {ts_sample}, channel {spec_ch_idx}: {e}") # Verbose
        # Return NaN array matching expected shape on error
        spec_window_samples = int(SPECTROGRAM_WINDOW_MS * fs / 1000)
        return np.full((len(SPECTROGRAM_FREQS), spec_window_samples), np.nan)
    finally:
        if lfp_data_memmap_worker is not None and hasattr(lfp_data_memmap_worker, '_mmap'):
            try: lfp_data_memmap_worker._mmap.close()
            except Exception: pass
...
def run_ripple_analysis(lfp_filepath, meta_filepath, channel_info_filepath,
                        ...
                        output_dir):
    """Main function to run the ripple and SPW detection analysis with parallel processing."""
    ...
    try:
        ...
        print("\n--- 1. Loading LFP Data ---")
        # Load initially just to get dimensions and fs, memmap object will be closed
        # Workers will create their own views later
        _, fs, n_channels, n_samples = load_lfp_data_memmap(lfp_filepath, meta_filepath)
        if fs is None or n_channels == 0 or n_samples == 0:
            raise ValueError("Failed to load LFP data or get valid dimensions/sampling rate.")
        duration_sec = n_samples / fs
        print(f"LFP data info: {n_channels} channels, {n_samples} samples ({duration_sec:.2f} seconds), Fs={fs} Hz")
        ...
        if not epoch_boundaries_samples:
             ...
             epoch_boundaries_samples = [(0, n_samples)]
        ...
        baseline_tasks = [(ch_idx, lfp_filepath, meta_filepath, non_rem_periods_samples, fs, n_channels)
                          for ch_idx in channels_to_process if ch_idx < n_channels] # Ensure ch_idx is valid

        # Run in parallel
        # backend="loky" is generally more robust
        ...
        for epoch_idx, (epoch_start_sample, epoch_end_sample) in enumerate(epoch_boundaries_samples):
            ...
            if epoch_duration_samples <= 0: continue

            all_ripple_events_by_epoch[epoch_idx] = {}
            all_spw_events_by_epoch[epoch_idx] = {}

            # Prepare tasks for channels relevant to this epoch
            epoch_tasks = []
            for ch_idx in channels_to_process:
                 if ch_idx in baseline_stats_per_channel: # Only process channels with valid baseline
                      ch_region = channel_region_map.get(ch_idx, 'Unknown')
                      epoch_tasks.append((ch_idx, ch_region, epoch_start_sample, epoch_end_sample,
                                          lfp_filepath, n_samples, n_channels, baseline_stats_per_channel[ch_idx], fs))

            if not epoch_tasks:
                 print(f"  No channels with valid baseline stats for epoch {epoch_idx}. Skipping event detection.")
                 continue

            print(f"  Detecting events for {len(epoch_tasks)} channels in parallel...")
            # Run event detection in parallel for this epoch
            ...
            for ch_idx, ripple_events, spw_events in epoch_results:
                 # Add epoch index back to event details before storing
            ...
            gc.collect() # Clean up memory after each epoch's parallel run


        # --- Post-Epoch Processing (using global event lists) ---
        # (Steps 6-9 are largely sequential logic operating on the collected global data)

        # 6. SPW-R Coincidence Check (CA1 only - using global lists)
        print("\n--- 6. Checking SPW-Ripple Coincidence for CA1 (Globally) ---")
        # ... (Keep the existing logic from the previous version for this step) ...
        ...
        for shank_idx in shanks:
            ...
            for region in target_regions:
                ...
                # else: print(f"  Shank {shank_idx}, Region {region}: No suitable reference site found.")
        ...
        for region in target_regions:
            ...
            averaged_timestamps[region] = pooled_times
        ...
        if ref_region in averaged_timestamps:
            ...
            for target_reg in target_check_regions:
                 ...
                 # else: print(f"    No global reference site for target {target_reg}.")
        ...
        # else: print(f"  Reference region {ref_region} has no global timestamps.")
        ...
        for region, timestamps in averaged_timestamps.items():
            ...
            spec_ch_idx = regional_ref_sites_indices[0] # Use first ref site for this region
            print(f"  Calculating spectrograms for region {region} using reference site {spec_ch_idx} LFP...")
            print(f"  Processing {len(timestamps)} timestamps for {region}...")

            # Prepare tasks for parallel spectrogram calculation
            spec_tasks = [(ts, spec_ch_idx, lfp_filepath, n_samples, n_channels, fs)
            ...
            # Filter out potential None results (errors in workers) and NaN arrays
            ...
            if valid_spectrograms:
                try:
                    # Stack and average valid spectrograms
                    avg_spec = np.nanmean(np.stack(valid_spectrograms, axis=0), axis=0)
                    averaged_spectrograms[region] = avg_spec
                    print(f"  Averaged {len(valid_spectrograms)}/{len(timestamps)} spectrograms for region {region}.")
        ...
        # 11. Save Results
        print("\n--- 11. Saving Results ---")
        # ... (Keep the existing saving logic, ensure allow_pickle=True for complex dicts) ...
