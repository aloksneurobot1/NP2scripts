# -*- coding: utf-8 -*-
"""
Created on Thu Feb 20 18:14:43 2025
...
#     # Check if the data size is a multiple of the number of channels
#     if data.size % num_channels!= 0:
#         # If not, trim the extra data
#         data = data[:-(data.size % num_channels)]
...
def filter_data(data, fs, cutoff_freq):
    ...
    taps = signal.firwin(numtaps=31, cutoff=normalized_cutoff_freq)
...
def compute_spectrogram(data, fs, window_size, step_size):
    ...
    stft = ShortTimeFFT(nfft=window_samples, fs=fs)  # Use nfft instead of nperseg

    # Compute spectrogram
    frequencies, times, spectrogram = stft.spectrogram(data, noverlap=window_samples - step_samples)  # Pass noverlap to spectrogram method
...
def score_sleep_states(pc1, theta_dominance, emg, buffer_size=10):
    ...
    return sleep_states
# def score_sleep_states(pc1, theta_dominance, emg):
#     """
#     Scores sleep states based on the provided metrics.
...
#     sleep_states = np.zeros_like(pc1, dtype=int)
#     sleep_states[(pc1 > nrem_threshold) & (emg < emg_threshold)] = 1  # NREM
#     sleep_states[(theta_dominance > rem_threshold) & (emg < emg_threshold)] = 2  # REM
#     sleep_states[(pc1 <= nrem_threshold) & (theta_dominance <= rem_threshold)] = 0  # Awake
# Parameters
original_fs = 2500
target_fs = 1250
cutoff_freq = 450
