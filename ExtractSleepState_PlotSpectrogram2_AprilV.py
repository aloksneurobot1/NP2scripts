# -*- coding: utf-8 -*-
"""
Created on Fri Apr 25 2025 (Visualization added, refined V5 - No LFP Plot)
...
def compute_emg(data, fs, window_size, step_size, n_ica_components=10, ica_epochs_duration=240, num_channels_for_ica=48):
    ...
    try:
        ...
        window_samples = int(round(window_size * fs))
        step_samples = int(round(step_size * fs))
        # Calculate num_segments based on the SPECTROGRAM times length, not EMG raw length
        # This ensures alignment if EMG calculation was successful
        # Need number of time points from outside, assume it matches spectrogram output length
        # Placeholder - this should align with spectrogram 'times' length passed implicitly
        # We will recalculate num_segments later if needed, or rely on length check

        # Let's calculate based on raw length for now, but check against pc1 length later
...
def score_sleep_states(pc1, theta_dominance, emg, times,
    ...
    # Thresholds are now calculated in main loop and passed if needed by plot,
    # but we still need them here for the logic. Recalculate robustly.
    try:
...
# *** UPDATED VISUALIZATION FUNCTION V5 (No LFP Plot) ***
def plot_scoring_overview(times, frequencies, spectrogram, pc1, theta_dominance, emg,
    ...
    """
    print("--- Generating Scoring Overview Plot (No Raw LFP) ---")
    ...
    try:
        ...
        # --- Panel 0: Spectrogram (0-20 Hz) 
        ax = axs[0]
        try:
            img1 = ax.pcolormesh(times, frequencies, spec_db, cmap='viridis', shading='gouraud', vmin=vmin, vmax=vmax)
        ...
        # --- Panel 1: Spectrogram (100-250 Hz) 
        ax = axs[1]
        try:
            img2 = ax.pcolormesh(times, frequencies, spec_db, cmap='viridis', shading='gouraud', vmin=vmin, vmax=vmax)
        ...
        # --- Save and Close Plot ---
        plot_filename = output_path / f'{base_filename}_ScoringOverview.svg' 
        plt.savefig(plot_filename, dpi=600, bbox_inches='tight')
        print(f"Saved scoring overview plot: {plot_filename}")
        plt.close(fig)

    except Exception as e:
        print(f"Error generating scoring overview plot V5 for {base_filename}: {e}")
        traceback.print_exc()
        plt.close('all')
...
for lfp_file_path in lfp_files:
    ...
    try: 
        ...
        if (times is not None and frequencies is not None and spectrogram is not None and
            ...
            plot_scoring_overview( # CALL THE UPDATED PLOT FUNCTION (V5)
