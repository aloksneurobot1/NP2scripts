# -*- coding: utf-8 -*-
"""
Created on Fri May 30 11:03:56 2025
...
# "manual": User will be prompted to select ref channels for CA1, CA2, CA3 via PSD plots.Sam's code suggestion.
# "automatic": CA1 uses PSD metric; CA2/CA3 use max ripple power.
REFERENCE_SELECTION_MODE = "manual" 
...
DOWNSAMPLING_FACTOR = 2 # Set to 1 to disable downsampling. As fs_orig=2500, this makes fs_eff=1250 Hz.
...
def select_ref_channel_by_manual_psd_visualization(
    ...
    n_total_channels,
    n_total_samples,
    region_channel_indices_on_shank, # List of channel indices for the current region on the current shank
    current_shank_idx, 
    current_region_name, # Name of the region (e.g., "CA1", "CA3")
    output_dir_psd_plots, # Directory to save PSD plots
    readMeta_func, 
    ...
    try:
        ...
        for ch_idx in region_channel_indices_on_shank:
            ...
            valid_channels_for_shank_region.append(ch_idx)
            gc.collect()
    ...
    plt.suptitle(f"Manual {current_region_name} Ref Channel Selection for Shank {current_shank_idx}", fontsize=16)
    plt.title(f"Target Fs for PSD: {current_fs_for_psd:.2f} Hz. Inspect traces and identify reference channel.", fontsize=10)
    plot_low_freq, plot_high_freq = plot_freq_range
    for ch_idx_plot in valid_channels_for_shank_region:
    ...
    plt.xlabel("Frequency (Hz)"); plt.ylabel("Z-scored Power Spectral Density (dB)")
    if valid_channels_for_shank_region : plt.legend(loc='upper right', fontsize='small')
    plt.grid(True, linestyle=':', alpha=0.7); plt.tight_layout(rect=[0, 0.03, 1, 0.95]) 
    plot_savename = output_dir_psd_plots / f"MANUAL_SELECTION_shank{current_shank_idx}_region{current_region_name}_PSDs.png"
    try:
...
def select_ca1_ref_by_automated_psd_metric(
    ...
    shank_ca1_channel_indices, current_shank_idx, # Added current_shank_idx for consistency in messaging
    ...
    # This function's logic is similar to the original select_ca1_ref_by_psd_per_shank
    # Key difference: it bandpass filters LFP to 100-250Hz BEFORE PSD, then Z-scores PSD within that band.
    meta = readMeta_func(meta_filepath)
    ...
    try:
        ...
        for ch_idx in shank_ca1_channel_indices:
            ...
            if q > 1: 
                ...
                else: warnings.warn(f"Auto PSD Sel CA1: Ch {ch_idx} shank {current_shank_idx}: Not enough for decimation."); # proceed with original Fs for this ch
            del lfp_segment_scaled

            # Bandpass filter for ripple band (100-250 Hz) BEFORE PSD
...
# ... (Data Loading, Signal Processing, Plotting, Worker functions remain the same as your previous version) ...
...
def run_ripple_analysis(lfp_filepath, meta_filepath, channel_info_filepath,
    ...
    try:
        ...
        # --- REFERENCE SITE SELECTION (MANUAL OR AUTOMATIC) ---
        if REFERENCE_SELECTION_MODE == "manual":
            ...
            for shank_idx_iter in shanks:
                ...
                for region_name_iter in target_regions:
                    shank_region_ch_df = region_channels_df[(region_channels_df['shank_index'] == shank_idx_iter) & (region_channels_df['acronym'] == region_name_iter)]
                    ...
                    print(f"    Prompting for {region_name_iter} on Shank {shank_idx_iter} (Channels: {region_ch_indices_on_shank})...")
            ...
            if not reference_channel_set: warnings.warn("MANUAL reference selection resulted in NO channels. Subsequent analysis might be empty or fail.")
        # If "automatic", reference sites are determined later.
        ...
        # ... (rest of Step 6 logic as before, using `use_noise_channel` and `dynamically_selected_noise_channel_idx`)
        ...
        # --- AUTOMATIC REFERENCE SITE SELECTION (if mode is "automatic") ---
        if REFERENCE_SELECTION_MODE == "automatic":
            print(f"\n--- Step A.8: Determining Reference Sites AUTOMATICALLY ---")
            _all_ripple_global_temp_for_ref_auto = {} # Use a different name to avoid conflict if manual mode ran
        ...
        # Ensure reference_sites is populated by now, either manually or automatically.
        ...
        # reference_channel_set was already defined if manual, or defined above if automatic.
        # If it's still empty here, it means no channels were selected by either method.
        ...
        # ... (unchanged, uses `reference_sites`) ...
        averaged_timestamps_by_state_epoch = {state: {} for state in STATES_TO_ANALYZE} 
        for state_code, state_name in STATES_TO_ANALYZE.items():
            print(f"  Processing state: {state_name}")
            ...
            for epoch_idx in sorted(list(relevant_epochs_for_state)):
                ...
                for region in target_regions:
                    regional_ref_indices_for_current_region = [
                        sites[region] for shank, sites in reference_sites.items() if region in sites and sites.get(region) is not None # check .get(region)
                    ]
...
if __name__ == "__main__":
    # ... (Tkinter file dialogs as before) ...
