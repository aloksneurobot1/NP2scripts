# -*- coding: utf-8 -*-
"""
Created on Mon Jun  2 17:50:41 2025 Multiple epoch avg Ripple-locked plots
...
# --- Main script logic (MODIFIED to allow multiple epoch inputs for NREM ripple-locked plots) ---
def main_ripple_analysis_and_visualization():
    ...
    if sleep_lkp:
        ...
        print("No sleep data loaded. Ripple-locked average plots will target 'AllData' if any ripples found, and NREM-specific plots will be skipped.")
        STATES_TO_ANALYZE = {} # Clear this so NREM-specific plotting is skipped if no sleep data
        ...
        for ep_idx, (ep_start, ep_end) in enumerate(processing_epochs): 
            ...
            for (shk, area_n), ref_ch in sel_ref_chs.items():
                ...
                for i_rip, ev_dict_original in enumerate(ep_rips_sorted):
                    ev_dict = ev_dict_original.copy() # Work with a copy
                    ...
                    if st_name_detect == "AllData" or curr_ev_st_code == st_code_detect:
                        ...
                        # Example LFP trace plotting logic 
    ...
    if all_ripples_final_list and depth_column_to_use and epoch_b_samp_loaded and STATES_TO_ANALYZE:
        ...
        for code, name in STATES_TO_ANALYZE.items():
            if name.upper() == "NREM":
                nrem_state_code_for_plot = code; nrem_state_name_for_plot = name; break
        ...
        if nrem_state_code_for_plot is not None:
            ...
            if chosen_epochs_input_str.strip() == '-1' or not chosen_epochs_input_str.strip():
                ...
                try:
                    ...
                    chosen_epoch_indices_for_plotting = [] # Renamed to avoid confusion
                    ...
                    if not valid_input_for_epochs or not chosen_epoch_indices_for_plotting:
                        ...
                        for current_chosen_epoch_idx in chosen_epoch_indices_for_plotting: # Loop through each chosen epoch
                            ...
                            for (shk_plot_idx, area_plot_name), ref_ch_plot_idx in sel_ref_chs.items():
                                ...
                                if ripples_for_avg.empty:
                                    print(f"    No NREM ripples in Epoch {current_chosen_epoch_idx} for RefCh {ref_ch_plot_idx} (S{shk_plot_idx} {area_plot_name}). Skipping plot for this ref channel/epoch.")
                                    continue
                                ...
                                if channels_to_display_df.empty: 
                                    print(f"    No channels from target regions found on Shank {shk_plot_idx} to display for RefCh {ref_ch_plot_idx}. Skipping plot.")
                                    continue
         ...
         print("Skipping Ripple-Locked Average LFP plots (no depth column found).")
    elif not epoch_b_samp_loaded :
         print("Skipping Ripple-Locked Average LFP plots (no user-defined epochs).")
    elif not STATES_TO_ANALYZE : 
         print("Skipping Ripple-Locked Average LFP plots (no sleep states defined to analyze).")
