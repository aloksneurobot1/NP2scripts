...
Focus on: Ripple-Locked Average LFP Plots with Plotly & Voltage Heatmap for a Chosen Epoch & NREM state.
"""
...
def plot_channel_psds(lfp_memmap_obj, fs, uv_scale_factor, 
    ...
    for i, ch_idx in enumerate(channel_indices_to_plot):
        ...
        if uv_scale_factor is not None: sig_f64 *= uv_scale_factor
        try:
            ...
            frequencies, psd_v = welch(sig_f64, fs=fs, nperseg=curr_nperseg)
        ...
        fig.add_trace(go.Scatter(x=frequencies[plot_mask_1_200Hz], y=psd_db_zscored[plot_mask_1_200Hz], mode='lines', 
                                 line=dict(width=1, color=plot_colors[i % len(plot_colors)]), opacity=0.7, name=ch_n_legend))
    ...
    fig.update_layout(title=f"{title_prefix}Welch PSD (1-200 Hz)", xaxis_title="Frequency (Hz)", 
                      yaxis_title=y_axis_title, font=dict(size=12), template="plotly_white", 
                      showlegend=True, legend_title_text=f'Channels (Avg Power in {ripple_freq_range_for_avg[0]}-{ripple_freq_range_for_avg[1]}Hz Band)', 
...
def plot_ripple_locked_avg_lfp_plotly( 
    ...
    if total_window_samples <= 0: # Check for valid window size
        print(f"    Calculated window is {total_window_samples} samples for {plot_context_title}. Skipping plot.")
        return
    ...
    if lfp_filter_range:
        ...
        if low_cut <= 0 : low_cut = 0.001 # Ensure positive for bandpass
        if low_cut < high_cut: # Ensure valid band
    ...
    for i_ch_plot, (_, ch_row) in enumerate(channels_df_for_plot.iterrows()):
        ...
        if segments_for_this_ch:
            ...
            except ValueError as e_avg: warnings.warn(f"Could not average LFP for Ch {ch_global_idx} in {plot_context_title}: {e_avg}")
...
def main_ripple_analysis_and_visualization():
    ...
    else: print("No sleep data loaded. Ripple-locked average plots may use all data if no specific state is filtered for."); # STATES_TO_ANALYZE remains as defined
    ...
    if all_ripples_final_list and depth_column_to_use and epoch_b_samp_loaded and STATES_TO_ANALYZE:
        ...
        nrem_state_code_for_plot = None
        nrem_state_name_for_plot = "NREM" 
        if STATES_TO_ANALYZE : 
            for code, name in STATES_TO_ANALYZE.items():
                if name.upper() == "NREM": # Case-insensitive check
                    nrem_state_code_for_plot = code
                    nrem_state_name_for_plot = name # Use the actual name from STATES_TO_ANALYZE
                    break
        
        if nrem_state_code_for_plot is not None:
            valid_epoch_indices_str = ", ".join(map(str, range(len(epoch_b_samp_loaded))))
            chosen_epochs_str = input(f"Enter Epoch Index(es) for NREM ripple-locked plots (e.g., 0 or 0,2,4 or -1 to skip) [0-{len(epoch_b_samp_loaded)-1}]: ")
            
            if chosen_epochs_str.strip() == '-1':
                print("Skipping NREM ripple-locked average plots by user choice.")
            else:
                try:
                    # Parse comma-separated input
                    chosen_epoch_indices = [int(x.strip()) for x in chosen_epochs_str.split(',') if x.strip()]
                    
                    ripple_df_for_plotting = pd.DataFrame(all_ripples_final_list) # Define once

                    for chosen_epoch_idx in chosen_epoch_indices:
                        if not (0 <= chosen_epoch_idx < len(epoch_b_samp_loaded)):
                            print(f"Invalid epoch index: {chosen_epoch_idx}. Skipping for this index.")
                            continue

                        print(f"\n  Generating for NREM, Epoch {chosen_epoch_idx}:")
                        for (shk_plot_idx, area_plot_name), ref_ch_plot_idx in sel_ref_chs.items():
                            ...
                            ripples_for_avg = ripple_df_for_plotting[
                                ...
                                (ripple_df_for_plotting['epoch_idx'] == chosen_epoch_idx) & # Use current chosen_epoch_idx
                                (ripple_df_for_plotting['state_code'] == nrem_state_code_for_plot)
                            ]
                            if ripples_for_avg.empty:
                                print(f"    No NREM ripples in Epoch {chosen_epoch_idx} for RefCh {ref_ch_plot_idx} (S{shk_plot_idx} {area_plot_name}). Skipping plot.")
                                continue
                            ...
                            channels_to_display_df = chan_df[
                                ...
                                (chan_df['acronym'].str.upper() == area_plot_name.upper()) 
                            ].sort_values(by=depth_column_to_use, ascending=False)
                            if channels_to_display_df.empty: continue

                            plot_filename_base = f"{base_fname}_{nrem_state_name_for_plot}_Ep{chosen_epoch_idx}_S{shk_plot_idx}A{area_plot_name}RefC{ref_ch_plot_idx}_RippleLockedAvg"
                            
                            plot_ripple_locked_avg_lfp_plotly( 
                                plot_context_title=f"{nrem_state_name_for_plot} Ripple-Locked Avg LFP - Shank {shk_plot_idx} {area_plot_name} (RefCh {ref_ch_plot_idx}) - Epoch {chosen_epoch_idx}",
                    ...
                    print("Invalid epoch index format entered. Expected comma-separated numbers or -1. Skipping NREM ripple-locked plots.")
        else:
            print("    NREM state not defined in STATES_TO_ANALYZE. Cannot generate NREM ripple-locked plots.")
    # ... (rest of the main function)
    elif not depth_column_to_use: print("Skipping Ripple-Locked Average LFP plots (no depth column found).")
