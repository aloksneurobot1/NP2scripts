# -*- coding: utf-8 -*-
"""
Created on Fri May 30 12:28:02 2025
...
from scipy.signal import butter, filtfilt as scipy_filtfilt, welch, decimate as scipy_decimate, iirnotch # Explicit imports for PSD function
...
REFERENCE_SELECTION_MODE = "automatic" # Options: "manual" or "automatic"
print(f"Configuring reference channel selection mode: {REFERENCE_SELECTION_MODE}")

# --- Notch Filter Configuration ---
APPLY_NOTCH_FILTER = True
NOTCH_FREQUENCIES = [60.0, 180.0]  # Frequencies to notch out (Hz)
NOTCH_Q_FACTOR = 60.0           # Quality factor for notch filters
...
def apply_notch_filters_on_data(data, fs, frequencies_to_notch, Q):
    """Applies a series of IIR notch filters to the data."""
    if not APPLY_NOTCH_FILTER or not frequencies_to_notch or data is None or data.size == 0:
        return data    
    nyquist = fs / 2.0
    processed_data = data.astype(np.float64) if not np.issubdtype(data.dtype, np.floating) else data.copy()

    for freq_to_remove in frequencies_to_notch:
        if freq_to_remove <= 0 or freq_to_remove >= nyquist:
            warnings.warn(f"Notch frequency {freq_to_remove} Hz is outside valid range (0, {nyquist} Hz) for fs={fs} Hz. Skipping this notch filter.")
            continue
        
        w0 = freq_to_remove / nyquist
        b_notch, a_notch = iirnotch(w0, Q)
        processed_data = signal.filtfilt(b_notch, a_notch, processed_data, axis=-1)
        # print(f"Applied notch filter at {freq_to_remove} Hz") # For debugging
        
    return processed_data

# --- Helper Function for MANUAL Reference Channel Selection (All Regions) ---
def select_ref_channel_by_manual_psd_visualization(
    lfp_filepath, meta_filepath, n_total_channels, n_total_samples,
    region_channel_indices_on_shank, current_shank_idx, current_region_name, 
    output_dir_psd_plots, readMeta_func, get_voltage_scaling_factor_func, 
    target_psd_fs=1250.0, plot_freq_range=(1.0, 200.0) 
):
    print(f"\n--- Manual {current_region_name} Reference Selection for Shank {current_shank_idx} ---")
    selected_channel_idx = None; all_channel_psds = {}; all_channel_freqs = {}
    valid_channels_for_shank_region = []
    meta = readMeta_func(meta_filepath)
    if meta is None: warnings.warn(f"PSD Sel (Manual {current_region_name}): No meta for shank {current_shank_idx}."); return None
    fs_orig_str = meta.get('imSampRate') or meta.get('niSampRate')
    if fs_orig_str is None: warnings.warn(f"PSD Sel (Manual {current_region_name}): No Fs for shank {current_shank_idx}."); return None
    fs_orig = float(fs_orig_str); uv_scale_factor = get_voltage_scaling_factor_func(meta)
    if not region_channel_indices_on_shank: warnings.warn(f"PSD Sel (Manual {current_region_name}): No channels for shank {current_shank_idx}."); return None
    if fs_orig <= 0: warnings.warn(f"PSD Sel (Manual {current_region_name}): Invalid Fs for shank {current_shank_idx}."); return None

    q = 1; current_fs_for_psd = fs_orig
    if fs_orig > target_psd_fs: q = max(1, int(round(fs_orig / target_psd_fs)))
    ...
    if n_total_samples <= 0: warnings.warn(f"PSD Sel (Manual {current_region_name}): Total samples {n_total_samples} for shank {current_shank_idx}."); return None
    ...
    if samples_to_load_orig_fs < current_fs_for_psd * 5 * q : warnings.warn(f"PSD Sel (Manual {current_region_name}): Not enough data for shank {current_shank_idx}."); return None
    ...
    try:
        ...
        for ch_idx in region_channel_indices_on_shank:
            ...
            # Apply Notch Filter
            lfp_segment_notched = apply_notch_filters_on_data(lfp_segment_scaled, fs_orig, NOTCH_FREQUENCIES, NOTCH_Q_FACTOR)
            del lfp_segment_scaled

            lfp_processed_for_psd = lfp_segment_notched
            if q > 1: 
                if len(lfp_segment_notched) > q * 20: 
                    lfp_processed_for_psd = scipy_decimate(lfp_segment_notched, q=q, ftype='fir', zero_phase=True)
                else: warnings.warn(f"PSD Sel Ch {ch_idx} shank {current_shank_idx}: Not enough for decimation.")
            del lfp_segment_notched
            
            nperseg_welch = int(2 * current_fs_for_psd) 
            if len(lfp_processed_for_psd) < nperseg_welch: continue
            try: frequencies, psd_values = welch(lfp_processed_for_psd, fs=current_fs_for_psd, nperseg=nperseg_welch)
            ...
            all_channel_psds[ch_idx] = zscored_psd_db; all_channel_freqs[ch_idx] = frequencies
            valid_channels_for_shank_region.append(ch_idx); gc.collect()
    except Exception as e: warnings.warn(f"PSD Sel (Manual {current_region_name}): Error processing shank {current_shank_idx}: {e}"); return None
    finally:
    ...
    if not valid_channels_for_shank_region: warnings.warn(f"PSD Sel (Manual {current_region_name}): No valid channels to plot for shank {current_shank_idx}."); return None
    valid_channels_for_shank_region.sort() 
    ...
    plot_low_freq, plot_high_freq = plot_freq_range; ax = plt.gca()
    for i, ch_idx_plot in enumerate(valid_channels_for_shank_region):
        if ch_idx_plot in all_channel_psds:
            freqs_plot = all_channel_freqs[ch_idx_plot]; psd_data_plot = all_channel_psds[ch_idx_plot]
            ...
            if np.any(freq_mask_for_plot):
                ax.plot(freqs_plot[freq_mask_for_plot], psd_data_plot[freq_mask_for_plot], label=f'Ch {ch_idx_plot}', linewidth=1, alpha=0.6)
    ax.set_xlabel("Frequency (Hz)", fontsize=12); ax.set_ylabel("Z-scored Power Spectral Density (dB)", fontsize=12)
    if valid_channels_for_shank_region:
        num_ch = len(valid_channels_for_shank_region); ncol_legend = 1
        if num_ch > 10: ncol_legend = 2
        if num_ch > 20: ncol_legend = 3
        ax.legend(loc='upper right', fontsize='small' if num_ch <=20 else 'x-small', title="Channels", ncol=ncol_legend)
    ax.spines['right'].set_visible(False); ax.spines['top'].set_visible(False)
    ax.grid(True, linestyle=':', alpha=0.5); plt.xlim(actual_plot_low_freq, actual_plot_high_freq)
    plt.tight_layout(rect=[0, 0.03, 1, 0.93]) 
    plot_savename = output_dir_psd_plots / f"MANUAL_SELECTION_shank{current_shank_idx}_region{current_region_name}_PSDs_OVERLAID.png"
    try: plt.savefig(plot_savename); print(f"\nPLOT SAVED: {plot_savename}")
    except Exception as e_save: print(f"Error saving PSD plot: {e_save}")
    plt.close() 
    print(f"Please open the saved plot to inspect {current_region_name} channel PSDs for shank {current_shank_idx}.")
    ...
    while True:
        try:
            user_input = input(f"Enter the selected {current_region_name} reference channel index for shank {current_shank_idx} (from list, or 'skip'): ")
            if user_input.strip().lower() == 'skip': selected_channel_idx = None; break
            potential_idx = int(user_input)
            if potential_idx in valid_channels_for_shank_region: selected_channel_idx = potential_idx; break
    ...
    if selected_channel_idx is not None: print(f"  PSD Sel (Manual {current_region_name}): Selected ref channel for shank {current_shank_idx}: {selected_channel_idx}")
    else: warnings.warn(f"PSD Sel (Manual {current_region_name}): User skipped selection for shank {current_shank_idx}.")
...
def select_ca1_ref_by_automated_psd_metric(
    ...
    selected_channel_idx = None; max_psd_metric = -np.inf
    meta = readMeta_func(meta_filepath)
    ...
    try:
        ...
        for ch_idx in shank_ca1_channel_indices:
            ...
            lfp_processed = lfp_segment_notched
            if q > 1: 
                if len(lfp_segment_notched) > q * 20: lfp_processed = scipy_decimate(lfp_segment_notched, q=q, ftype='fir', zero_phase=True)
            ...
            try: frequencies, psd_values = welch(lfp_ripple_filtered, fs=current_fs_for_psd, nperseg=nperseg_welch)
            except ValueError: continue
            ...
            if not np.isnan(metric) and metric > max_psd_metric: max_psd_metric = metric; selected_channel_idx = ch_idx
            gc.collect()
...
# --- Data Loading Functions, Signal Processing, Plotting, Worker functions ---
...
def plot_lfp_trace_with_event(lfp_filepath, n_total_channels, n_total_samples,
    ...
    try:
        ...
        lfp_segment_orig_raw = lfp_memmap_plot[plot_start_sample_orig:plot_end_sample_orig, channel_idx_to_plot].astype(np.float64)
        
        lfp_segment_orig_scaled = (lfp_segment_orig_raw * uv_scale_factor) if uv_scale_factor is not None else lfp_segment_orig_raw
        
        # Apply Notch Filter
        lfp_segment_orig = apply_notch_filters_on_data(lfp_segment_orig_scaled, fs_orig, NOTCH_FREQUENCIES, NOTCH_Q_FACTOR)

        if pad_left > 0 or pad_right > 0: lfp_segment_orig = np.pad(lfp_segment_orig, (pad_left, pad_right), mode='reflect')
            
        y_label = "LFP (\u00b5V)" if uv_scale_factor is not None else "LFP (ADC units)"
        time_axis_ms = (np.arange(len(lfp_segment_orig)) - window_samples_half_orig - pad_left) * (1000 / fs_orig)

        plt.figure(figsize=(10, 6))
        plt.plot(time_axis_ms, lfp_segment_orig, label=f'LFP Ch {channel_idx_to_plot} (Notched)', color='black', linewidth=0.75)
        
        ripple_filtered_overlay = apply_fir_filter(lfp_segment_orig.copy(), RIPPLE_FILTER_LOWCUT, RIPPLE_FILTER_HIGHCUT, fs_orig) # Will use notched data
...
def _calculate_baseline_worker(ch_idx, lfp_filepath, meta_filepath, non_rem_periods_samples_orig, fs_orig, n_channels, uv_scale_factor_val):
    ...
    try:
        ...
        lfp_ch_full_orig_raw = lfp_data_memmap_worker[:, ch_idx].astype(np.float64)
        lfp_ch_full_orig_scaled = (lfp_ch_full_orig_raw * uv_scale_factor_val) if uv_scale_factor_val is not None else lfp_ch_full_orig_raw
        if uv_scale_factor_val is None: warnings.warn(f"Ch {ch_idx}: No scaling factor, baseline power in AU^2")

        # Apply Notch Filter
        lfp_ch_full_orig = apply_notch_filters_on_data(lfp_ch_full_orig_scaled, fs_orig, NOTCH_FREQUENCIES, NOTCH_Q_FACTOR)
...
def _detect_events_worker(ch_idx, ch_region, epoch_start_sample_orig, epoch_end_sample_orig,
    ...
    try:
        ...
        lfp_ch_epoch_orig_raw = lfp_data_memmap_worker[epoch_start_sample_orig:epoch_end_sample_orig, ch_idx].astype(np.float64)
        lfp_ch_epoch_orig_scaled = (lfp_ch_epoch_orig_raw * uv_scale_factor_val) if uv_scale_factor_val is not None else lfp_ch_epoch_orig_raw
        if uv_scale_factor_val is None: warnings.warn(f"Ch {ch_idx}, Ep {epoch_start_sample_orig}: No scaling factor, power in AU^2")

        # Apply Notch Filter
        lfp_ch_epoch_orig_notched = apply_notch_filters_on_data(lfp_ch_epoch_orig_scaled, fs_orig, NOTCH_FREQUENCIES, NOTCH_Q_FACTOR)
        del lfp_ch_epoch_orig_scaled

        if DOWNSAMPLING_FACTOR > 1: lfp_ch_epoch_down = signal.decimate(lfp_ch_epoch_orig_notched, q=DOWNSAMPLING_FACTOR, axis=0, ftype='fir', zero_phase=True)
        else: lfp_ch_epoch_down = lfp_ch_epoch_orig_notched
        del lfp_ch_epoch_orig_notched
        
        # ... (Rest of ripple and SPW detection logic uses lfp_ch_epoch_down) ...
...
def _calculate_spectrogram_worker(ts_sample_orig, spec_ch_idx, lfp_filepath, n_samples_orig, n_channels, fs_orig, uv_scale_factor_val):
    ...
    try:
        ...
        segment_orig_raw = lfp_data_memmap_worker[start_sample_orig:end_sample_orig, spec_ch_idx].astype(np.float64)
        segment_orig_scaled = (segment_orig_raw * uv_scale_factor_val) if uv_scale_factor_val is not None else segment_orig_raw
        
        # Apply Notch Filter
        segment_orig_notched = apply_notch_filters_on_data(segment_orig_scaled, fs_orig, NOTCH_FREQUENCIES, NOTCH_Q_FACTOR)
        del segment_orig_scaled

        if pad_left_orig > 0 or pad_right_orig > 0:
            try: segment_orig_padded = np.pad(segment_orig_notched, (pad_left_orig, pad_right_orig), mode='reflect')
            except ValueError: segment_orig_padded = np.pad(segment_orig_notched, (pad_left_orig, pad_right_orig), mode='edge')
        else:
            segment_orig_padded = segment_orig_notched
        del segment_orig_notched

        if DOWNSAMPLING_FACTOR > 1 and segment_orig_padded.size > DOWNSAMPLING_FACTOR * 20: 
            segment_down = signal.decimate(segment_orig_padded, q=DOWNSAMPLING_FACTOR, axis=0, ftype='fir', zero_phase=True)
        else: segment_down = segment_orig_padded
        del segment_orig_padded
...
# --- Main Analysis Function `run_ripple_analysis` ---
def run_ripple_analysis(lfp_filepath, meta_filepath, channel_info_filepath,
    ...
    try:
        ...
        if dynamically_selected_noise_channel_idx is not None:
            ...
            if dynamically_selected_noise_channel_idx >= n_channels or dynamically_selected_noise_channel_idx < 0:
                ...
                try:
                    ...
                    lfp_ch_noise_orig_raw = lfp_noise_memmap[:, dynamically_selected_noise_channel_idx].astype(np.float64)
                    lfp_ch_noise_orig_scaled = (lfp_ch_noise_orig_raw * uv_scale_factor) if uv_scale_factor is not None else lfp_ch_noise_orig_raw
                    
                    # Apply Notch Filter to noise channel data
                    lfp_ch_noise_orig_notched = apply_notch_filters_on_data(lfp_ch_noise_orig_scaled, fs_orig, NOTCH_FREQUENCIES, NOTCH_Q_FACTOR)
                    del lfp_ch_noise_orig_scaled

                    if DOWNSAMPLING_FACTOR > 1: lfp_ch_noise_down = signal.decimate(lfp_ch_noise_orig_notched, q=DOWNSAMPLING_FACTOR, axis=0, ftype='fir', zero_phase=True)
                    else: lfp_ch_noise_down = lfp_ch_noise_orig_notched
                    del lfp_ch_noise_orig_notched
        ...
        # ... (Steps 5, 6, 6.5, 7, A.8 (if auto), 9, 10, 11, CSV/NPY saving)
