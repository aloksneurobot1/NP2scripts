...
-- MODIFIED to include user prompts to filter plots by neuron type and/or brain region.
...
- A detailed report of each neuron's firing rate and CV in every epoch (CSV).
- A summary report of the average CV for each cell type/area group per epoch (CSV).
...
# --- MODIFICATION: New helper function for user input ---
def get_user_filter(options, item_name):
    """
    Displays available options and prompts the user for a comma-separated list
    of items to include in the analysis.
    """
    if not options:
        return []
    ...
    for item in selected_items:
        try:
            # Find the original-cased option
...
def generate_epoch_rasters():
    ...
    # Convert cell_type to a categorical for stable sorting before filtering
    units_df['cell_type'] = pd.Categorical(units_df['cell_type'], categories=CELL_TYPE_ORDER, ordered=True)

    # --- MODIFICATION: Get user input for filtering ---
    ...
    # 2. --- Setup Output ---
    # Create a unique directory name based on filters
    filter_name_parts = []
    if selected_types:
        filter_name_parts.extend(selected_types)
    if selected_regions:
        filter_name_parts.extend(selected_regions)
    ...
    for epoch in epochs:
        ...
        for (area, cell_type), mean_cv in cv_summary.items():
            all_epochs_cv_summary.append({
                'epoch_index': epoch_idx, 'acronym': area,
                'cell_type': cell_type, 'mean_cv': mean_cv
            })
        ...
        ax_cv.set_xlim(-0.5, epoch_duration)
    ...
    if all_epochs_firing_rates:
        ...
        print(f"\nSUCCESS: Firing rate and CV report saved to {fr_csv_path.name}")
