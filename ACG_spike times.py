# -*- coding: utf-8 -*-
"""
Created on Thu May  8 12:34:05 2025
fitting ACG from https://github.com/petersenpeter/CellExplorer/blob/main/calc_CellMetrics/fit_ACG.m
Burst index Antonio,
...
def process_kilosort_directory(kilosort_folder):
    ...
    try: # Outer try block for directory-level errors
        ...
        if not npy_file_exists and not sec_file_exists:
             print(f"    DEBUG: os.path.exists returned False for both spike_times.npy and spike_times_sec.npy. Listing directory contents for {kilosort_folder}:")
             try:
        ...
        try:
            if sec_file_exists:
                ...
                spike_times_seconds = spike_times_samples / SAMPLING_RATE_HZ
        ...
        if len(spike_times_seconds) != len(spike_clusters):
            print(f"  ERROR: Mismatch spike times ({len(spike_times_seconds)}) /clusters ({len(spike_clusters)}) length. Skipping directory.")
            return None
        ...
        if os.path.exists(brain_regions_file):
            try: brain_regions_df = pd.read_csv(brain_regions_file, index_col='global_channel_index'); print(f"    Loaded brain region mapping from: {brain_regions_file_name}");
            except Exception as e: print(f"    Warning: Error processing brain regions file '{brain_regions_file}': {e}")
        else: print(f"    Warning: Brain regions file '{brain_regions_file_name}' not found. Acronym and region name will be empty.")
        ...
        min_time_rec = np.nanmin(spike_times_seconds); max_time_rec = np.nanmax(spike_times_seconds)
        if np.isnan(min_time_rec) or np.isnan(max_time_rec) or max_time_rec <= min_time_rec : recording_duration = 0.0; print("    Warning: Cannot determine valid recording duration.")
        else: recording_duration = max_time_rec - min_time_rec
        if recording_duration <= 0: print(f"    Warning: Recording duration non-positive or zero ({recording_duration:.2f}s). Firing rates may be inf or nan.")

        fit_failures = 0; skipped_fits = 0
        for i, cluster_id_val in enumerate(unique_clusters_all): # Renamed to avoid conflict
            ...
            if not metrics_row.empty:
                ...
                is_good = (contam_rate is not None and contam_rate < MAX_CONTAM_RATE and
                ...
                if is_good:
                    ...
                    if n_spikes >= MIN_SPIKES_FOR_ACG_FIT:
                        acg_counts_calc, bin_centers_calc = calculate_acg(cluster_spike_times_seconds, ACG_BIN_SIZE_MS, ACG_WINDOW_MS) # Renamed
                        if acg_counts_calc is not None and bin_centers_calc is not None:
                            popt_fit, _ = fit_acg(acg_counts_calc, bin_centers_calc) # Renamed
                            if popt_fit is not None:
                                tau_rise_parameter = popt_fit[1]
                                acg_fit_params = popt_fit
                            else: fit_failures += 1;
                        else: skipped_fits +=1 # ACG calc returned None
                    else: skipped_fits +=1

                    cell_type = classify_cell_type_full(trough_to_peak_ms, tau_rise_parameter)

                    good_cluster_info = {
                        'cluster_id': int(cluster_id), 'cell_type': cell_type,
                        'firing_rate_hz': round(firing_rate, 3) if pd.notna(firing_rate) else None,
                        ...
                        'num_spikes': n_spikes, 'peak_channel_index_0based': peak_channel_index,
                        ...
                        'acg_fit_rsquare': None,
                        'shank_index': None, 'distance_from_entry_um': None, 'acronym': None, 'brain_region_name': None,
                        'channel_depth_um_fallback': None,
                        'spike_times_sec': cluster_spike_times_seconds # Stored for NPY
                    }
                    ...
                    if brain_regions_df is not None:
                        if peak_channel_index in brain_regions_df.index:
                            ...
                            good_cluster_info['acronym'] = str(region_info.get('acronym')) if pd.notna(region_info.get('acronym')) else None # Ensure string if not None
                            good_cluster_info['brain_region_name'] = str(region_info.get('name')) if pd.notna(region_info.get('name')) else None # Ensure string if not None
                        # else: print(f"    Debug: Peak channel {peak_channel_index} for cluster {cluster_id} not in brain_regions_df.index.") # Optional debug
        ...
        if good_clusters_final:
            ...
            output_npy_file_name = f'good_clusters_processed_{output_base_name}_CellExplorerACG.npy';
            ...
            try:
                ...
                print(f"\n  Per-directory data (including spike times per cluster in NPY) saved to: {os.path.basename(output_npy_path)}");
            except Exception as e: print(f"  Error saving per-directory NPY file: {e}")

            try:
                # Define column order for CSV, EXCLUDING spike_times_sec
                cols_order_csv = [
                    'cluster_id', 'cell_type', 'acronym', 'brain_region_name',
                    'firing_rate_hz', 'trough_to_peak_ms', 'burst_index',
                    'acg_tau_rise', 'acg_tau_decay', 'acg_tau_burst', 'acg_refrac', 'acg_fit_rsquare',
                    'isi_violation_percent_manual', 'isi_viol_rate_allensdk',
                    'contamination_rate', 'l_ratio', 'num_spikes',
                    'peak_channel_index_0based', 'shank_index',
                    'distance_from_entry_um', 'channel_depth_um_fallback'
                ]
                good_clusters_df_for_csv = good_clusters_df.copy()
                if 'spike_times_sec' in good_clusters_df_for_csv.columns:
                    good_clusters_df_for_csv = good_clusters_df_for_csv.drop(columns=['spike_times_sec'])

                cols_present_in_df_for_csv = [col for col in cols_order_csv if col in good_clusters_df_for_csv.columns]
                additional_cols_not_in_order = [col for col in good_clusters_df_for_csv.columns if col not in cols_present_in_df_for_csv]
                final_csv_cols = cols_present_in_df_for_csv + additional_cols_not_in_order
                
                good_clusters_df_for_csv = good_clusters_df_for_csv[final_csv_cols]
                output_csv_path = os.path.join(kilosort_folder, f'good_clusters_processed_{output_base_name}_CellExplorerACG.csv');
                good_clusters_df_for_csv.to_csv(output_csv_path, index=False, float_format='%.4f');
                print(f"  Per-directory metrics summary (excluding spike times) saved to: {os.path.basename(output_csv_path)}");
        ...
        print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
        print(f"!!! CRITICAL FILE NOT FOUND error processing directory {kilosort_folder}: {e}");
        print(f"!!! Skipping to next directory.");
        print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"); return None
    except Exception as e:
        print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
        print(f"!!! UNHANDLED ERROR processing directory {kilosort_folder}: {e}");
        print(f"!!! Skipping to next directory."); traceback.print_exc();
        print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"); return None
