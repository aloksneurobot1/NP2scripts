...
Focus on: Ripple-Locked Average LFP Plots with Plotly & Voltage Heatmap
"""
...
# --- User-provided find_swr function (MODIFIED to return noise_excluded_count) ---
def find_swr(lfp, timestamps, fs=2500, thresholds=(2, 5), durations=(20, 40, 500),
             freq_range=(100, 250), noise=None):
    # ... (initial parts of the function are the same) ...
    ...
    if low_f >= high_f: return [], None, None, None, 0 # Added 0 for noise_excluded_count
    ...
    try:
        ...
        print(f"Filter creation error in find_swr: {e}"); return [], None, None, None, 0
    ...
    final_kept_before_noise_rejection = kept_arr[mask] # Keep track before noise rejection
    ...
    noise_excluded_count = 0 # Initialize counter
    final_kept_after_noise_rejection = final_kept_before_noise_rejection.copy() # Start with all valid duration events

    if noise is not None and len(noise) == len(lfp):
        try:
            # Process noise channel the same way as LFP for envelope
            ...
            for i, (start, end) in enumerate(final_kept_before_noise_rejection): # Iterate through events that passed duration filter
                if start >= end or end > len(noise_env_z): continue # Should not happen if start < end from merged
                if not np.any(noise_env_z[start:end] > high_thresh): 
                    valid_event_indices_after_noise.append(i)
                else:
                    noise_excluded_count += 1 # Increment if rejected
            ...
            print(f"Noise rejection error: {e}. Proceeding with events before noise rejection for this call.")
            # If error, noise_excluded_count remains 0 for this call, and all duration-valid events are kept
            # No, we should rather say that no events are excluded if noise rejection fails
            noise_excluded_count = 0 # Reset if noise rejection itself failed
    
    ripple_events = []
    if final_kept_after_noise_rejection.any(): # Check if any events remain after noise rejection
...
def plot_channel_psds(lfp_memmap_obj, fs, uv_scale_factor, 
                      ...
                      ripple_freq_range_for_avg, # Parameter name used in main
                      title_prefix=""):
    nperseg = int(2 * fs) 
    fig = go.Figure()
    try:
    ...
    for i, ch_idx in enumerate(channel_indices_to_plot):
        ...
        # Use the passed ripple_freq_range_for_avg for calculating average power
    ...
    fig.update_layout(title=f"{title_prefix}Welch PSD (1-200 Hz)", xaxis_title="Frequency (Hz)", 
                      yaxis_title="Z-scored Power Spectral Density (dB)", font=dict(size=12), 
...
def plot_ripple_locked_avg_lfp_plotly( 
    ...
    window_samples_half = int((window_ms_around_peak / 2) / 1000 * fs)
    total_window_samples = window_samples_half * 2
    if total_window_samples == 0:
        print(f"    Calculated window is 0 samples for {plot_context_title}. Skipping plot.")
        return
    ...
    if lfp_filter_range:
        ...
        if low_cut < high_cut:
            try: b_lfp, a_lfp = butter(3, [low_cut, high_cut], btype='band'); can_filter = True
            except ValueError as e: print(f"    Filter ({lfp_filter_range} Hz) error for ripple-locked avg: {e}. Using raw LFP.")
        else: print(f"    Invalid LFP filter range ({lfp_filter_range} Hz) for ripple-locked avg. Using raw LFP.")
    else: print(f"    No LFP filter range specified for ripple-locked avg. Using raw LFP.")
    ...
    for i_ch_plot, (_, ch_row) in enumerate(channels_df_for_plot.iterrows()):
        ...
        for peak_samp_abs in all_ripple_peak_samples_abs:
            ...
            if start_abs >= 0 and end_abs <= lfp_memmap_obj.shape[0] and end_abs > start_abs :
                ...
                if can_filter and b_lfp is not None and a_lfp is not None:
                    try: segment_processed = filtfilt(b_lfp, a_lfp, segment_raw)
                    except ValueError: segment_processed = segment_raw 
                else: segment_processed = segment_raw
        ...
        if segments_for_this_ch:
            ...
            except ValueError as e_avg: warnings.warn(f"Could not average LFP segments for Ch {ch_global_idx} for {plot_context_title}: {e_avg}")
    ...
    if finite_heatmap_data.size > 1:
        ...
        if np.isclose(p_low, p_high):
            center = p_low; current_std = np.std(finite_heatmap_data) if len(finite_heatmap_data)>1 and np.std(finite_heatmap_data) > 1e-9 else 1.0
            span = max(abs(center*0.1), 0.1 * current_std); span = max(span, 0.5) 
            zmin_h, zmax_h = center - span, center + span
    ...
    fig.update_layout(
        title=f"{plot_context_title} ({plot_title_filter_text})",
        xaxis_title="Time from Ripple Peak (ms)",
...
# --- Main script logic (MODIFIED to track and print noise-excluded ripples) ---
def main_ripple_analysis_and_visualization():
    ...
    shank_overview_plots_d = output_d / f"{base_fname}_shank_ripple_avg_LFP_plots"; os.makedirs(shank_overview_plots_d, exist_ok=True)
    ...
    else: print("No sleep data loaded. Ripple-locked average plots will be for 'AllData' if any ripples found."); STATES_TO_ANALYZE = {}
    ...
    if not dg_df.empty:
        ...
        plot_channel_psds(lfp_mm, fs_eff, uv_sf, dg_idxs, chan_df, 
                          ripple_f_range, # Pass ripple_f_range for avg power in legend
    ...
    # Initialize total noise excluded ripples count
    ...
    for st_code_detect, st_name_detect in states_iter_for_detection.items():
        ...
        for ep_idx, (ep_start, ep_end) in enumerate(processing_epochs): 
            ...
            for (shk, area_n), ref_ch in sel_ref_chs.items():
                ...
                # Unpack the new noise_excluded_count from find_swr
                ...
                total_noise_excluded_ripples += excluded_this_call # Accumulate

                print(f"      Found {len(ep_rips)} raw rips (after noise rejection for this call) in ep {ep_idx} for Ch {ref_ch}.")
                if excluded_this_call > 0:
                    print(f"        ({excluded_this_call} ripples excluded by noise channel for this segment/channel)")
                ...
                for i_rip, ev_dict in enumerate(ep_rips_sorted):
                    ev_dict['abs_start_sample'] = ep_start + ev_dict['epoch_rel_start_sample'] # Use correct key after pop
                    ev_dict['abs_peak_sample'] = ep_start + ev_dict['epoch_rel_peak_sample']
                    ev_dict['abs_end_sample'] = ep_start + ev_dict['epoch_rel_end_sample']
                    # epoch_rel_... are already in ev_dict from find_swr if it returned them, or should be event['start_sample']
                    # Correcting based on find_swr returning start_sample relative to input LFP
                    rel_start = ev_dict.pop('start_sample')
                    rel_peak = ev_dict.pop('peak_sample')
                    rel_end = ev_dict.pop('end_sample')
                    ev_dict['abs_start_sample'] = ep_start + rel_start
                    ev_dict['abs_peak_sample'] = ep_start + rel_peak
                    ev_dict['abs_end_sample'] = ep_start + rel_end
                    ev_dict['epoch_rel_start_sample'] = rel_start
                    ev_dict['epoch_rel_peak_sample'] = rel_peak
                    ev_dict['epoch_rel_end_sample'] = rel_end
                    ...
                    if st_name_detect == "AllData" or curr_ev_st_code == st_code_detect:
                        ...
                        # ... (Plotting individual ripple LFP traces - same as before) ...

    # --- Generate Ripple-Locked Average LFP Plots (NREM, First Epoch) ---
    if all_ripples_final_list and depth_column_to_use and epoch_b_samp_loaded: 
        print("\n--- Generating NREM Ripple-Locked Average LFP Plots (First Epoch Only) ---")
        ripple_df_for_plotting = pd.DataFrame(all_ripples_final_list)
        first_epoch_idx_for_plot = 0 
        nrem_state_code_for_plot = None; nrem_state_name_for_plot = "NREM" 
        if STATES_TO_ANALYZE : 
            for code, name in STATES_TO_ANALYZE.items():
                if name == "NREM": nrem_state_code_for_plot = code; break
        if nrem_state_code_for_plot is not None:
            for (shk_plot_idx, area_plot_name), ref_ch_plot_idx in sel_ref_chs.items():
                ...
                ripples_for_avg = ripple_df_for_plotting[
                    ...
                    (ripple_df_for_plotting['epoch_idx'] == first_epoch_idx_for_plot) &
                    (ripple_df_for_plotting['state_code'] == nrem_state_code_for_plot) ]
                if ripples_for_avg.empty:
                    print(f"    No NREM ripples in Epoch {first_epoch_idx_for_plot} for RefCh {ref_ch_plot_idx} (S{shk_plot_idx} {area_plot_name}). Skipping plot.")
                    continue
                all_ripple_peaks_for_avg = ripples_for_avg['abs_peak_sample'].tolist()
                channels_to_display_df = chan_df[(chan_df['shank_index'] == shk_plot_idx) & (chan_df['acronym'].str.upper() == area_plot_name.upper())].sort_values(by=depth_column_to_use, ascending=False)
                if channels_to_display_df.empty: continue
                plot_filename_base = f"{base_fname}_NREM_Ep{first_epoch_idx_for_plot}_S{shk_plot_idx}A{area_plot_name}RefC{ref_ch_plot_idx}_RippleLockedAvg"
                plot_ripple_locked_avg_lfp_plotly( 
                    plot_context_title=f"NREM Ripple-Locked Avg LFP - Shank {shk_plot_idx} {area_plot_name} (RefCh {ref_ch_plot_idx}) - Epoch {first_epoch_idx_for_plot}",
                    all_ripple_peak_samples_abs=all_ripple_peaks_for_avg, channels_df_for_plot=channels_to_display_df,
                    ref_channel_for_highlighting=ref_ch_plot_idx, lfp_memmap_obj=lfp_mm, fs=fs_eff, uv_scale_factor=uv_sf,
                    depth_col_name=depth_column_to_use, output_dir=shank_overview_plots_d, 
                    base_filename_prefix=plot_filename_base, uv_scale_factor_present=uv_sf_present,
                    lfp_filter_range=(1, 200), window_ms_around_peak=400 )
    ...
    # --- Print total noise excluded ripples ---
    if sel_noise_ch not in [None, -1]:
        print(f"\n--- Noise Exclusion Summary ---")
        print(f"Total number of potential ripple events excluded due to noise channel activity: {total_noise_excluded_ripples}")

    # --- Data Saving --- 
    print("\n--- Data Saving ---")
    # ... (Data saving part - same as before) ...
